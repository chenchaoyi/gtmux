...
**一眼看清哪个 coding agent 在等你 —— 横跨所有 tmux 会话 —— 然后直接跳过去。**
...
- **手机 app**：同一套雷达搬到 iOS，agent 需要你时锁屏推送，点一下就能回复。
...
- **[手机与远程访问](docs/phone.md)**：iOS app、`gtmux serve`，以及用 Tailscale 或 `gtmux tunnel` 从任意网络连回 Mac。
...
二进制；菜单栏和手机 app 都只是同一份 `gtmux agents --json` 的消费方。名字里的 “g” 取自 Go。
