...
很容易搞不清：哪个在**等你**拍板、哪个还在**运行**、哪个刚**干完**。

gtmux 就是那台雷达。它盯着你 tmux 里的 agent，一眼让你看清谁需要你，再落到那个
...
有些工具会**孵化** agent、把 agent 放进 git worktree 沙盒里（claude-squad、uzi、
dmux……）。gtmux 不一样，它**不运行你的 agent**，而是**你已有 tmux 之上的雷达加遥控器**。
...
**直接在终端 tab 里（不经 tmux）启动的 agent 不会被检测到**，这是有意为之，不是 bug。
...
GitHub 直取，所以即便压缩包走了镜像，用于校验的 checksum 仍锚定在 GitHub。可用
...
- **⠿ 运行**：忙着，别打扰。
- **⏸ 等待**：任务中途卡在**你**这儿等权限或确认，排到最顶，一眼看到谁要你拍板。
...
- **状态**取自 agent 自己写的 pane 标题：开头的盲文加载符（`⠋⠙⠹…`，多数 agent TUI
  会转的那个）表示**运行中**，Claude Code 的 `✳` 表示**空闲**。会转加载符的 agent 都通用。
- **是哪个 agent**：按前台命令（`claude`、`codex`、`gemini`、`aider`、`opencode`……）
  或标题里的名字匹配。
- 用 **`~/.config/gtmux/agents.json`** 扩展或覆盖：一个
...
所以 gtmux-core 始终是唯一数据源。CLI 保持零 cgo，app 是唯一的原生构建。Release 附带
...
在你的设备之间建一张私有 mesh 网络，绕开公司 Wi-Fi 的客户端隔离，公司、家里都能用。
...
> **同一 Wi-Fi 却连不上 Mac？** 公司/访客 Wi-Fi 常**隔离客户端**（手机和 Mac 被挡），
...
配对二维码**。手机 app → **配对 → 扫一扫** → 任意网络即连。（没装 `cloudflared`？它会
提示帮你 `brew install`。）
...
（默认 `en`）或 `$GTMUX_LANG`。一切显式调用，无 shell hook，任何 shell 都能用。
...
**每个 pane 之前的输出（scrollback）也会像快照一样一起回来**，前提是让 resurrect
抓取它。`tmux.conf` 推荐：
...
是生产端，由 Claude Code 来跑，不用你跑，纯凭事件**时序**写状态，不读消息内容就能区分
权限请求与空闲提醒。
