...
export const RELEASE_NOTES: ReleaseNote[] = [
  ...
  },
  {
    ...
    en: [
      ...
      '- HQ\'s console no longer ends at its last clear. A seam marks where one session ended and the next began, and a control above it reaches the session before, as far back as the record goes.',
    ...
    zh: [
      '对话记得更久',
      '- HQ 的对话不再止于上一次 clear。上一段会话结束、下一段开始的地方有一道分隔线，线上方能翻到更早的会话，记录有多远就翻多远。',
  ...
  },
  {
    ...
    en: [
      'Now on iPad',
      '- The same app, one listing. On an iPad the radar is a sidebar and the session, HQ or All panes you open fills the pane beside it — nothing pushed, tap another row and it switches.',
      '- The HQ page keeps what is waiting on you beside the conversation; All panes is a grid; the knowledge base reads list beside entry.',
      '- Hardware keyboard: hold ⌘ to see the commands — ↑↓⏎ along the radar, ⌘1–9 to a row, ⌘⇧H HQ, ⌘F search, ⌘K type.',
      '- The demo shows the iPad layout too.',
      'Thirty-odd releases since the version on your phone. Here is what you can use.',
      'Act on a session by long-pressing it',
      '- No need to open it: answer its question, tell it to carry on, interrupt the turn, hand it to HQ, jump to it on the Mac, or read the diff.',
      '- The press answers back: the row arms while you hold it, and there is a tap the moment it is recognised.',
      '- Reachable by VoiceOver: every action in that menu.',
      'The HQ page, rebuilt',
      '- What matters first: what is owed to you, then what HQ did while you were away, then the context.',
      '- Its work is visible: dispatches, reclaims and notes, grouped by when they happened, with the quiet between them named.',
      '- Its documents have a home: the situation board, the knowledge base and usage, each a tile carrying a live figure.',
      '- The knowledge base is searchable: hundreds of entries, by title, id or topic.',
      'New: a usage view',
      '- Where your quota stands: which plan window is tightest, how long until it resets, who is burning it, what the machine has left.',
      '- Codex too: its quotas alongside Claude\'s.',
      'Reading and typing',
      '- More screen: full screen no longer reserves room for a hidden status bar, and the bar above the terminal is three bands instead of four — about sixteen more lines.',
      '- A board you can scan: sections and panes stay folded until you open one.',
      '- Drafts survive: a half-typed message is waiting when you come back, and input history follows the project you are in.',
      '- A failed send explains itself: it says why, and keeps what you wrote.',
      'The Lock Screen card',
      '- Sharper: the session that needs you, its actual question, and how long it has waited — and it no longer goes stale.',
      '- Bilingual: it follows your system language.',
      'The rest is a long run of fixes and polish.',
    ],
    zh: [
      'iPad 版来了',
      '- 同一个 app、同一个商店条目。iPad 上雷达是侧栏，打开的会话、HQ、所有 pane 在旁边的主区里显示，不跳页，点另一行就换。',
      '- HQ 页把该你拍板的事放在对话旁边；所有 pane 是卡片网格；知识库列表和正文并排。',
      '- 接了键盘按住 ⌘ 看命令：↑↓⏎ 在雷达里走，⌘1–9 直达某行，⌘⇧H 打开 HQ，⌘F 搜索，⌘K 输入。',
      '- 演示模式也是 iPad 的排布。',
      '你手机上那一版之后更新了三十多次，这里是能直接用上的部分。',
      '长按会话直接处理',
      '- 不用点进去：长按一个会话即可回它的问题、让它继续、打断当前这一轮、转给 HQ、在 Mac 上切过去，或查看它改了什么。',
      '- 按得出手感：按住时该行缩起，认出长按的瞬间有一次震动。',
      '- 旁白可达：菜单里每一条动作都能被读屏读到。',
      'HQ 页重做',
      '- 重点提前：打开先看到欠你的事和 HQ 做过的事，背景信息放在后面。',
      '- 动作可查：派活、回收、记账按时间成组，组与组之间标出间隔。',
      '- 资料归位：态势板、知识库、用量各占一张卡片，卡上带实时数值。',
      '- 知识库可搜：几百条内容按标题、编号、主题检索。',
      '新增用量页',
      '- 额度可见：哪个窗口最紧、多久重置、谁在消耗、机器还剩多少。',
      '- 覆盖更全：Codex 的额度与 Claude 的并列显示。',
      '阅读与输入',
      '- 屏幕更大：全屏不再为状态栏留白，终端上方由四层压到三层，多出十余行输出。',
      '- 板子好扫：态势板改为提纲，分节与 pane 默认折叠，按需展开。',
      '- 草稿不丢：消息写到一半切走仍在，输入历史按项目区分。',
      '- 失败有交代：发送未成功时说明原因，并保留你写的内容。',
      '锁屏卡片',
      '- 信息更准：先显示谁在等、等的什么、等了多久，内容不再滞后于实际。',
      '- 支持中文：卡片文案跟随系统语言。',
      '其余为大量问题修复与体验打磨。',
    ],
  },
  {
    version: '1.0.14',
    en: [
      'The Lock Screen card speaks Chinese now. The whole app was bilingual except the one surface you see without opening anything.',
      ...
      'The Lock Screen card leads with the session that needs you, its actual question, and how long it has waited — and it stopped going stale.',
    ],
    zh: [
      '锁屏卡片现在说中文了。整个 app 都是双语，唯独这块「不打开 app 就会看到」的地方一直是英文。',
      'HQ 页顶部那块读着读着折走之后再也回不来，现在回到顶部它就回来了。',
      '长按会话现在有回应了：识别的那一刻有一次触感，按住时行会明显地「攒」起来。',
      '长按菜单按类别分开了。「打断它」——唯一会停掉正在进行的那一轮——不再和「看改动」长得一模一样。',
      '菜单里每一条动作现在 VoiceOver 都够得到。整张卡片以前是一个整体，读屏只能读到「这张卡」。',
      '锁屏卡片改成先说等你的那一个：会话名、它问的那句原话、等了多久；内容也不会再过时。',
    ],
  },
  {
    version: '1.0.13',
  ...
  },
  {
    ...
    en: [
      ...
      'The long-press menu separates its actions into groups. "Interrupt it" — the one that stops a turn in progress — no longer looks exactly like "See the changes".',
      'Every action in that menu is reachable by VoiceOver. The whole sheet used to be a single element, so a screen reader could reach the card and nothing inside it.',
      'The lock-screen card leads with the session that needs you, its actual question, and how long it has waited — instead of a count. Its ground carries the status colour, so "something needs me" reads before a word does.',
      'The lock-screen card stopped going stale: a card that has just started gets the current state at once, a phone that reconnects gets it again, and a card whose Mac you switched away from is no longer pushed to forever.',
    ],
    zh: [
      '长按会话现在有回应了：识别的那一刻有一次触感，按住时行会明显地「攒」起来；中途松手它弹回原状。',
      '长按菜单按类别分开了。「打断它」——唯一会停掉正在进行的那一轮——不再和「看改动」长得一模一样。',
      '菜单里每一条动作现在 VoiceOver 都够得到。整张卡片以前是一个整体，读屏只能读到「这张卡」，里面什么都摸不到。',
      '锁屏卡片改成先说等你的那一个：会话名、它问的那句原话、等了多久，而不是一个数字。底色跟着状态走，你还没读字就知道有事找你。',
      '锁屏卡片不再显示过时的内容：新建的卡立刻拿到当前状态，手机重连时补一次，而你切走的那台 Mac 也不会再对着一张不存在的卡永远推下去。',
    ],
  },
  {
