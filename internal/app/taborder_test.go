...
func TestShrinksTabOrder(t *testing.T) {
	full := []string{"日常更新", "docs", "dev-workspace", "cli", "main"}
	cases := []struct {
		name       string
		next, prev []string
		want       bool
		...
		{"pure removal", []string{"docs", "main"}, full, true},
		{"unchanged", full, full, false},
		{"reorder, same set", []string{"main", "cli", "docs", "dev-workspace", "日常更新"}, full, false},
