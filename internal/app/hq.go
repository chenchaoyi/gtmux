...
`,
	...
	"environment.md":    "# Environment / network\n\n_Whether an agent launch needs the local proxy is EXPLICIT — gtmux never probes the network (TUN vs a double-VPN can't be told apart: the port listens either way, a direct request 403s either way). Set it per network with `gtmux config agent-proxy off|on|<url>`, or the `GTMUX_AGENT_PROXY` env var (overrides config — wire it to your network switch)._\n\n- **off** (default) — launch BARE: office intranet, or Clash in TUN mode (traffic is transparent at the network layer, direct reaches the model API).\n- **on** — proxy via `http://127.0.0.1:<agentProxyPort, 7897>`: home behind a double-VPN, where a direct launch 403s.\n\n**The proxy (when on) covers ONLY gtmux's OWN launch path** (`gtmux spawn` / `hq` / `adopt` / `restore`). A bare `send-keys \"claude\"` you type by hand starts the agent OUTSIDE that path. ALWAYS dispatch with `gtmux spawn`. 起 agent 是否走代理是显式设置,gtmux 不探测;办公网/TUN 用 off 裸起,家里双层VPN 用 on(7897)。切网络用 `gtmux config agent-proxy` 或 `GTMUX_AGENT_PROXY` 环境变量。\n\n_Record the per-network rules here (which SSID = which mode)._\n",
}
