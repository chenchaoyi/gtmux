...
// opencodePluginJS is the gtmux bridge plugin for opencode: a JS module that maps
// opencode's plugin events onto gtmux hook events. The plugin's shell (Bun's `$`)
// inherits the opencode process env — including TMUX_PANE — so `gtmux hook`
// resolves the pane it runs in. bin is the resolved gtmux binary path.
func opencodePluginJS(bin string) string {
	bt := "`" // JS template-literal backtick for Bun's shell ($)
	var b strings.Builder
	b.WriteString("// gtmux hook bridge for opencode — generated by `gtmux install-hooks --agent opencode`.\n")
	b.WriteString("// Maps opencode plugin events onto gtmux hook events so waiting/done, receipt-\n")
	b.WriteString("// verified dispatch, and notifications light up. Do NOT edit — re-run the\n")
	b.WriteString("// installer to regenerate. A missing/renamed event just no-ops (gtmux falls\n")
	b.WriteString("// back to its screen read; absence of an event is never a failure).\n")
	b.WriteString("export const GtmuxHook = async ({ $ }) => {\n")
	b.WriteString("  const hook = (event) => $" + bt + bin + " hook --agent opencode ${event}" + bt + ".quiet().catch(() => {});\n")
	b.WriteString("  return {\n")
	b.WriteString("    \"session.idle\": async () => { await hook(\"Stop\"); },\n")
	b.WriteString("    \"permission.asked\": async () => { await hook(\"PermissionRequest\"); },\n")
	b.WriteString("    \"permission.replied\": async () => { await hook(\"PostToolUse\"); },\n")
	b.WriteString("    \"session.created\": async () => { await hook(\"SessionStart\"); },\n")
	b.WriteString("    \"session.deleted\": async () => { await hook(\"SessionEnd\"); },\n")
	b.WriteString("    \"session.error\": async () => { await hook(\"SessionEnd\"); },\n")
	b.WriteString("    \"session.compacted\": async () => { await hook(\"PreCompact\"); },\n")
	b.WriteString("  };\n")
	b.WriteString("};\n")
	return b.String()
}
