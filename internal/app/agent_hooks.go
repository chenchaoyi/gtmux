...
func opencodePluginJS(bin string) string {
	...
	b.WriteString("// Maps opencode's `event` + `chat.message` plugin hooks onto gtmux hook events so\n")
	b.WriteString("// waiting/done, receipt-verified dispatch, and notifications light up. Do NOT edit —\n")
	b.WriteString("// re-run the installer to regenerate. A missing/renamed event just no-ops (gtmux\n")
	...
	b.WriteString("    \"session.created\": \"SessionStart\",\n")
	b.WriteString("    \"session.deleted\": \"SessionEnd\",\n")
	b.WriteString("    \"session.error\": \"SessionEnd\",\n")
	b.WriteString("    \"session.compacted\": \"PreCompact\",\n")
	b.WriteString("    \"permission.asked\": \"PermissionRequest\",\n")
	b.WriteString("    \"permission.replied\": \"PostToolUse\",\n")
	b.WriteString("  };\n")
	b.WriteString("  return {\n")
	b.WriteString("    event: async ({ event }) => {\n")
	b.WriteString("      const token = MAP[event && event.type];\n")
	b.WriteString("      if (token) await fire(token);\n")
	b.WriteString("    },\n")
	b.WriteString("    \"chat.message\": async (_input, output) => {\n")
	b.WriteString("      const parts = (output && output.parts) || [];\n")
	b.WriteString("      const text = parts.filter((p) => p && p.type === \"text\").map((p) => p.text).join(\"\\n\").trim();\n")
	b.WriteString("      if (text) await submit(text);\n")
	b.WriteString("    },\n")
	b.WriteString("  };\n")
	b.WriteString("};\n")
	return b.String()
