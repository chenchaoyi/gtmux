...
func TestKnowledgeDismissLeavesATrace(t *testing.T) {
	asHQ(t)
	key := "pitfalls/noise"
	if err := AppendCandidate(Candidate{At: 100, Topic: "pitfalls", Key: key,
...
func TestCustomTopicWholeLoop(t *testing.T) {
	...
	hqHome := state.HQHome()
	t.Chdir(t.TempDir())
	if rc := CmdCapture([]string{"the etl-pipeline repo needs schema pinning", "@datasets"}, noHeader); rc != 0 {
		t.Fatal("capture under a declared topic must be accepted")
	}
	if rc := CmdCapture([]string{"whatever", "@undeclared"}, noHeader); rc == 0 {
		t.Fatal("capture under an undeclared topic must refuse")
	}
	// Capture accepts environment now too (the 5-vs-6 drift is closed).
	if rc := CmdCapture([]string{"the office network needs the proxy", "@environment"}, noHeader); rc != 0 {
...
func TestKnowledgeAddConsumesSeveralKeysAsOneLesson(t *testing.T) {
	...
	for _, k := range []string{"pitfalls/n1", "pitfalls/n2"} {
		_ = AppendCandidate(Candidate{At: 500, Topic: "pitfalls", Key: k, Lesson: k, Seq: 50})
	}
