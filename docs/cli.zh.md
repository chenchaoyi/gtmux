...
**在哪个窗口跑。** 不带参数时，`gtmux hq` 沿用中控已有的窗口：在跑就切过去，agent 退出了就在原窗口重新拉起，
两者都没有才自己建一个 tmux 会话并开 tab。「原窗口重新拉起」是有意粘住的，但你换了地方它还粘着：
以前跑过中控的窗口仍带着标记和启动目录，于是在任何地方敲 `gtmux hq` 都会把中控拉回那里。
要换地方就直说：`--pane %N` 在那个 pane 里启动（得是空着的 shell，会先 cd 到中控目录），
`--here` 在你正敲命令的这个 pane 里，`--new-pane` 把当前窗口拆一个新 pane 在里面启动。
三者都会把中控的身份挪到新 pane，旧窗口不再被认作中控；而只要有中控真的在跑，三者都会拒绝，中控只有一个。
...
跑了三周的会话不再被读成本周的消耗；`--json` 在 `history` 里带最近七天，在 `history.activity` 里带账本这一整年（每个有输出的日子、自账本第一天起的累计、峰值、连续天数）。
再多一行 `Σ 累计 … 自 … · 峰值 … · 连续 …` 一句话说这一年；`gtmux usage --activity` 把它画成手机和 Mac 阅读窗上那张日历热力格（周在横向、周一到周日在纵向、GitHub 那五档绿；终端多宽就画多少周，认 `COLUMNS`）：

```
Token 活动   最近 26 周
累计 70.8M · 峰值 5.3M · 连续 14 天（最长 25 天）
...
`--acts` 只留中控**自己做的事** —— 派活、回收、记知识、轮换、自检、蒸馏 ——
去掉把它敲醒的那些投递记录（`wake-delivered` / `wake-dropped`）。
这跟手机「参谋长动作」一节走的 `GET /api/hq/events?acts=1` 是同一道分割，
菜单栏卡片里「它做了」那一行数的也是这一批；真机上量过，投递记录比动作多四十倍，
所以「中控今天干了什么」就是一句 `gtmux events --since 24h --acts`，不用翻。
和所有过滤读法一样，它不算消费。
...
**但丢掉不等于闭嘴。** 靠日志上报的 agent 只要你不用它，它自己就会安静下来：上一次读数的窗口
翻页，它的行全部消失 —— 这跟 gtmux 坏掉长得一模一样。所以 Codex 读不到窗口、而你这一周里
又用过它时，它会得到属于自己的一行，而不是凭空不见：

```
● claude session              9% 已用   重置 Sep 7 at 9:09pm
● claude week (all models)   50% 已用   重置 Sep 11 at 10:59pm
○ codex  上次报告的窗口已经过去 —— codex 把额度写在会话日志里，跑一轮就能重新读到
```

`gtmux usage` 的页脚在它那点宽度里标一下同一件事（`codex 未知`），原因留给这条命令说。
一周都没用过的 agent 则完全不出声：不跑 Codex 的人，不该被告知 Codex 的事。

`gtmux limits` 会列出每一个窗口。**其余每个地方都只显示每个套餐一个 —— 最紧的那个**，
因为那些地方有的是一行、不是一张表：Codex 的窗口加进来之后，`gtmux usage` 的末行超过了
100 个字符，手机头部那一行则在数字中间被截断（「Fable 11…」）——
而百分比恰恰是最不能被截断的东西。留最紧的那个是对的，因为一句摘要回答的问题是
「我现在站在哪儿」。注意这**刻意**不同于告警的规矩：告警会忽略 5 小时窗口，
因为为一个自己会重置的窗口打断你是噪音；但当它就是最紧的那个时，**显示**它才是答案。

**每个窗口都写明属于谁的套餐，第一个 agent 的也不例外** —— `claude session`、
`codex session`，绝不会让一个光秃秃的 `session` 和一个带前缀的并排站。
不带前缀的标签挨着带前缀的，读起来像「通用的那个 + 旁边一个特例」，而事实恰好相反。
`spawn` 的飞行前检查打印的就是这条告警，所以它说的是这活真正要计费的那个套餐。因为这会起一个进程，结果会**缓存**
