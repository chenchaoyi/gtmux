...
`gtmux share code <id>` 给的是同样的访问权，但拆成一个不含任何秘密的地址，加一个短码：
...
对方能粘贴就发链接。粘不了就用短码：粘不进去的浏览器，或者你在电话里念给他。短码在十分钟
内能打开那条链接一次，用过就废了，链接本身照常有效。
...
就清掉脚本存储」的规则。重新打开那条链接就能恢复，短码不行，因为短码只能用一次。所以想让对
方以后能自己回来，就把链接发给他。终端把 token 存在 `~/.config/gtmux/remotes.json` 里，你不
吊销、他不删，它就一直在。
