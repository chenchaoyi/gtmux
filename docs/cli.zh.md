...
目前以 Claude 为主（其它 agent 的日志还不带用量）；hook 在每个生命周期事件上评估 ——
工具驱动的工作期间接近实时，一次长时间静默的生成则在它下一个事件时结算。
...
本地估算给不了的那个数字：**你的套餐还剩多少**。gtmux 靠 headless 地跑 agent
**自己的** `/usage` 命令拿到（`claude -p "/usage"`）—— 真实的服务端数据，
用户被认可的命令，不是逆向出来的接口。因为这会起一个进程，结果会**缓存**
