...
对方打开那个地址，把码输进去。终端里同一个码用 `gtmux attach <host> --code 96Z-NCC`，
它会为那台 host 记下来，之后直接 `gtmux attach <host>`。这个码 10 分钟内能打开那条链接一次，交出去的是链接自己的
token、自己的 pane 范围和过期时间 —— 所以一个码永远扩不了权，链接一吊销，为它生成的码
全部作废。容易听错的字符不在字母表里（`I`、`L`、`O`、`U`），输回来的时候大小写、连字符和
空格都不计较。

六位之所以够短，是因为猜的次数被卡住了：同一个来源一分钟错 10 次、所有来源加起来一分钟
错 60 次，serve 就不再作答，并把这次拒绝记进日志。输对的那次不计数，所以打错一次再输对，
不会因此被挡。
