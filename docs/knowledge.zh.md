...
| `hq/LOCAL.md` | **你的**长期规矩 | 你手写。gtmux 从不覆盖 |
| `hq/notes/board.md` | HQ 当前对舰队的判断，不算知识 | HQ |
| `knowledge/machine.md` | 这台机器上每个 agent 都该知道的那几条 | gtmux，自动生成 |

最后一行是最容易让人懵的地方，因为这下有两个都叫 `knowledge` 的目录。`hq/knowledge/`
是**仓库**：几百条条目，只有 HQ 读写。`~/.config/gtmux/knowledge/machine.md` 是**出货口**：
只装「写给整台机器」那一档的副本。删掉它，下次同步会重新生成。
...
**落地。** 晋升只是写好简报，落地才是 gtmux 真把它送到位。日常最要紧的是「本机」这一档：
...
改是 HQ 的活，那些动词是它的工具，多数情况下你不用自己敲。你可能真会想自己用的有三条：
