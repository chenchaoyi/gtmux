...
这下有两个都叫 `knowledge` 的目录。它们的区别不在于一个是原件一个是副本，而在于**读它们的
是两拨人，后一拨根本看不见前一个目录**。

`hq/knowledge/` 是 HQ 自己的。别的 agent 不会去翻它，你在某个项目里开的 Claude Code 或
Codex 压根不知道有这么个目录。但每个 agent 启动时一定会读自己那份全局配置，Claude Code
读的是 `~/.claude/CLAUDE.md`。所以一条 HQ 学到的教训，想真正作用到正在干活的那个会话，
唯一的路就是进到那份配置里，而 `machine.md` 是这条路上的中转站。

没有它是什么代价，一个例子就够。本机把 `rm` 换成了会问一句 y/n 的版本，agent 在非交互
环境里执行它会静默失败：命令看起来跑过了，文件还在。这条教训只记在知识库里，拦不住任何人，
因为新开的会话从来不读知识库。推出去写进 `machine.md`、再进到各 agent 的配置里，它才会
出现在每个新会话的第一个回合。
