...
- **两扇窗的入口是角色横幅右端的两枚小标**（`态势板` · `知识库`，`HQReader.swift`，各开一扇窗）。**只用图标、不带文字**，名字留在 tooltip 和无障碍标签里：它们是入口不是控件，而横幅那一行本来就存在 —— 早先是卡片下方一对整宽按钮，为两个次级入口吃掉了弹层里最贵的一整行（本节开头那条：最贵的像素不能卖给退一步就看得到的东西）。中控没在跑时这一行仍是原来那句说明。这是对早期「点了只跳 pane、不另开面板」的**有意修订**，边界重画在**读 / 驱动**之间而不是**屏幕**之间：
  ...
  - **窗口顶上有一行「记忆」**：多大、几份本地快照、以及**有没有任何东西把它带离这块盘** ——
    最后那句是 CLI 自己的原话（`gtmux hq --records --json`），不是转述，两块屏不会对同一件事
...
- **卡片展开成参谋长报告**（menubar-hq-report，2026-09-14；图纸 `mockup/menubar-hq/`）：卡头不变，点它仍跳 HQ pane；右端加一个 ⌄，在描边面板**里面**打开**手机那张报告表**（MOBILE §17，`hqHeaderModel.ts`），是搬不是重画（`HQCardReport.swift`，纯函数，两种语言都有测试钉住）：键列（`机器 · 知识库 · 态势板 · 它做了`，宽度按语言量过：en 80 / zh 48，所有行共用一个宽度）加值列；是门的行带 `›`，点开读窗口对应那页。**没话说的行不画。**`知识库`把手机的「待你」行折进门里（`478 条 · 7 条待你带走 · 最久 16 天`；弹层只有 420pt，手机是两行），**只有超过 `gtmux doctor` 那条两周线才变琥珀**。`它做了`是中控最近一天自己动作的计数（`gtmux events --since 24h --acts`，从 app 自己的目录读，永远不会推进 HQ 的水位），按手机那个固定顺序（看分量，不看次数）。**机器行只在红档时升到第一行**，那时变红、允许折两行（截断一个读数比折行更糟，何况它就是卡片自动打开的原因）；琥珀时留在原位、变琥珀；正常时灰字，仍是门。为什么不是画过的另外两个方向：卡片下面常驻一排三扇带字的门要占弹层最贵的 30pt（本节已经否过一次的东西），也放不下「待你 / 它做了」；照手机页做一扇独立 HQ 窗口会给最常做的动作（跳 pane）多加一次点击，还需要现在没有的 CLI 读口；真要做，这张表就是那扇窗的左栏。**进入注意态时卡片自己打开**（HQ 等你 · worker 等你 · 资源瓶颈），只开一次——退出红态不关它，一直红也不重开读者关掉的；手动开合会记住（`hq.cardExpanded`）。都正常：一行，和以前一样。横幅上那两枚小标留作快捷，带字的行才是能被发现的入口（2026-09-14：「kb 与 board 不太明显」）。**数据只在展开时读**：展开时读四样、弹层开着每 60 秒一次；收起时除了徽章本来就跑的资源轮询什么都不花，那个轮询现在留整份快照而不只留档位。
- **导出件上锁，表单先问锁再问放哪**（hq-export-passphrase，2026-09-14；图纸 `mockup/hq-export/`）：「导出…」先弹一张表单 —— 口令、再输一次、一个「显示」开关，和**一行**提示只说那件该改的事（「太短」「两次不一样」「好」）—— 然后才是保存面板，默认名 `gtmux-hq-<日期>.tar.gz.age`。先给文件起名选位置、再因为口令太短被拒，顺序是反的。一个勾选框把口令记进这台 Mac 的钥匙串，默认打勾：锁是给要出门的那份副本上的，不是给坐在自己键盘前的人添的麻烦（司令原话：「电脑是自己的，不应该给自己设置太多麻烦」），所以第二次导出打开就是一行「用这台 Mac 钥匙串里记着的口令 · 换一个…」，一下就导。表单最后是确认页：路径、大小、一句怎么打开（`gtmux hq --import`，或任何 age 工具）以及 gtmux 不留副本；失败时原样给出 CLI 的 stderr。口令走标准输入递给 CLI，永远不走命令行参数。档案那一行多一句「上次导出 3 天前，已上锁」—— 这是 gtmux 唯一能担保的机外副本。别的都不动：盘上的知识库、快照、态势板照旧，也没有任何界面对库里存了什么念叨。
