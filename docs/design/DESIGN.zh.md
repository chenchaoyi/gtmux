...
- **导出件上锁，表单先问锁再问放哪**（hq-export-passphrase，2026-09-14；图纸 `mockup/hq-export/`）：「导出…」先弹一张表单 —— 口令、再输一次、一个「显示」开关，和**一行**提示只说那件该改的事（「太短」「两次不一样」「好」）—— 然后才是保存面板，默认名 `gtmux-hq-<日期>.tar.gz.age`。先给文件起名选位置、再因为口令太短被拒，顺序是反的。一个勾选框把口令记进这台 Mac 的钥匙串，默认打勾：锁是给要出门的那份副本上的，不是给坐在自己键盘前的人添的麻烦（司令原话：「电脑是自己的，不应该给自己设置太多麻烦」），所以第二次导出打开就是一行「用这台 Mac 钥匙串里记着的口令 · 换一个…」，一下就导。表单最后是确认页：路径、大小、一句怎么打开（`gtmux hq --import`，或任何 age 工具）以及 gtmux 不留副本；失败时原样给出 CLI 的 stderr。口令走标准输入递给 CLI，永远不走命令行参数。记忆那一行多一句「上次导出 3 天前，已上锁」—— 这是 gtmux 唯一能担保的机外副本。别的都不动：盘上的知识库、快照、态势板照旧，也没有任何界面对库里存了什么念叨。
