...
the iPad; no step asked "and what does this do on the iPad". The demo fell behind the real
radar twice for the same reason. A surface that is not on the list drifts, quietly.

## Keeping drift out: structure first, then the checklist

The checklist catches forgetting; structure catches copying. Both are needed:

1. **One implementation, several presentations.** When the same thing appears on two
   surfaces it is one component plus a prop, never two copies. Phone and iPad:
   ...
   share data contracts (`agents --json`, `/api/digest`), not code, so `DESIGN.md` and
