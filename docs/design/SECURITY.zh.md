...
- 配对二维码里短期内含 token（局域网直连场景）；隧道场景用的是一次性配对码而非 token（见 §3）。
...
- 推送经托管 APNs 中继转发，中继能看到通知内容（agent 名 / 任务文本）。不想暴露就关推送。
...
- 是一次性、5 分钟过期、单次使用的 enroll code，没有长期 token 的权限。
...
- 可信网络优先用「局域网」模式（`serve`，局域网直连，完全不走云）；Anywhere 隧道留给真在外面时。
...
- 应用层端到端加密 + 零知识中继（D1 选项 c）：让 Cloudflare 边缘 / 中继都看不到明文
