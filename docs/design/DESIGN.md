...
- **无状态角标**（v2）：HQ 恒在监视，无 idle/working 之分——头像不带角标，并删 agent 名/时长等 session 语言。副题=**情报头条**（hq-meta-layer）：从舰队确定性合成的一句参谋长结论——点名"谁在等你 + 其余 N 个正常"，或平静时"都正常 · 无需你介入"（有 worker 等你转琥珀，HQ 自己等你转红）。**不再用舰队光点条**（那排匿名色点与下方列表/摘要计数完全重复，连"是哪个 pane 在等"都答不了）。HQ 自身等你拍板时**整卡琥珀描边 + 副题红字**；状态栏计数仍含 HQ，但**通知/推送/锁屏 tally 已把 HQ 排除**。**HQ 不再像 worker 那样推送任何通知**（waiting/done 一律不推——#557 修的就是「列表里已找不到 HQ，却还收到它的 worker 式推送」的矛盾）；用参谋长自己语气的独立通知类别仍是待办（见 hq-meta-layer）。
