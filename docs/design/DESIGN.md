...
- **自己的 `NSStatusItem`**，与 agent 状态项并列、不叠加。agent 状态项的字形表达「舰队里最紧急的
  **agent** 状态」，把机器状态叠上去会破坏 §9「颜色只表达状态」并稀释 waiting 红。
- **开着才出现，关掉即消失** —— 关闭时菜单栏与这个功能存在之前完全一样，零占用。
- **不可被隐藏**：`空闲时隐藏`（§2 模式 3）管的是「舰队空闲别烦我」;这是用户**自己选择**让机器保持的
  状态，忘掉它正是这个指示器要防的事故。macOS 对录屏和定位就是这么处理的。
- **点它就是关它**：小菜单 = 档位 · 已开启多久 · 变红原因（若有）· **关闭服务器模式**（主操作）。
- **形状承载存在，颜色只承载注意**：字形 `laptopcomputer`；健康时中性 `#8E8E93`，
  仅在护栏触发（电池低于 30% / 守护缺失 / 设置失效）时转权威红 `#EF4444`。
  **不引入新色、不用 macOS 录屏的橙**（§9 调色板里没有橙，余光下会与 waiting 红打架）。
- **零动效**：§10 全产品只允许一处动画（idle→waiting 单次脉冲）；一个常驻指示器若脉冲将不可忍受。
...
FileVault**。手机端只读 + 可关，**永不可远程开启**（见 MOBILE §7）。
