...
- **无状态角标**（v2）：HQ 恒在监视，无 idle/working 之分——头像不带角标，并删 agent 名/时长等 session 语言。副题=**情报头条**（hq-meta-layer）：从舰队确定性合成的一句参谋长结论——点名"谁在等你 + 其余 N 个正常"，或平静时"都正常 · 无需你介入"（有 worker 等你转琥珀，HQ 自己等你转红）。**不再用舰队光点条**（那排匿名色点与下方列表/摘要计数完全重复，连"是哪个 pane 在等"都答不了）。HQ 自身等你拍板时**整卡琥珀描边 + 副题红字**；状态栏计数仍含 HQ，但**通知/推送/锁屏 tally 已把 HQ 排除**（HQ 只在真要你拍板时才通知，routine done 静默——见 hq-meta-layer）。
- **指挥所三区**：状态条（舰队计数 + 订阅窗口% + 资源告警）· 舰队态势板（`/api/digest`，waiting 的 ask 琥珀高亮、ctx%）· 命令台（与 HQ 对话 + 快捷命令 简报/谁在等我/该我拍板/派活）。
- **红线**：命令台只对 HQ 说话，HQ 再驱动舰队（`send/spawn`）；只建议、不代拍板；长按舰队行跳该 worker Detail。
