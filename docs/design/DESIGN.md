...
- **弹层高度随舰队增长**（2026-08-11）：列表高度 = **实测内容高度**，再由 `min(820, 屏高−280)`
  封顶。此前只设了 `.frame(maxHeight:)`，而 SwiftUI 的 `ScrollView` **没有固有内容高度** ——
  它在任何尺寸下都"满意"，于是在按内容自适应的 `NSHostingController` 里塌向最小值，那个慷慨的
  上限从未被用到：一块能放 17 行的屏幕上只显示 3 行。**上限回答的是"你最多能多高"，而当时没有任何
  东西在回答"你想要多高"**。内容自己上报高度，弹层才能长到那么高。
