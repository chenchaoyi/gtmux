...
4. Native, restrained, trustworthy: this is a developer tool. No rainbow gradients, no glowing shadows, no marketing copy.
...
- **The colour channel is reserved for status alone**; never use it to tell agent types apart (see §6).

---

## 2. The menu-bar status item (NSStatusItem), the most important surface

A 16pt glyph that must be readable in a second in peripheral vision, colour-blind, and on light / dark / tinted menu bars. **Waiting must be carried by shape and glyph; colour alone is never enough.**

### Glyph (recommended: Shape-shift, the shape changes with status)

The status item's glyph changes shape with the "most urgent status", and the colour follows:
...
2. Grouped list: the section order is fixed, needs you (waiting) → errored → working →
   ...
   - "Errored" is a section inside the four-status language (2026-08-17): the status language still has exactly four entries; errored describes
   ...
   - **Never fold errored into "needs you"**: that section means "the agent is asking you a question you can answer", and an error
     is no question; you can't answer it, you can only go fix the environment. Folding it in dilutes the iron rule
...
- B · standalone command palette (recommended as the hotkey entry): a centred, Raycast-style panel, large hit areas, keyboard-first, a shortcut bar at the bottom
  (`↑↓ 选择 · ⏎ 跳转 · ⌘1–9 直达`, i.e. ↑↓ select · ⏎ jump · ⌘1 to 9 direct).
...
explanation card; the system dialog alone, thrown straight at the user, is the wrong first contact.
...
gtmux supports agents inside tmux and also agents running directly in a native terminal (no tmux).
...
A row whose session has no terminal client attached says so on the row, and clicking it opens a window and attaches.
Why: clicking a row means "take me there", but a session with no terminal client attached (dispatched with `--headless`, or
...
that could not exist, and nothing happened, silently. All the user could conclude was "click-to-jump is broken".

- The criterion is the client count (`session_attached`); how the session was started does not enter into it. Measured: the `restart-feed` window
...
- Presentation: such rows get a small neutral tag `no window / 无窗口`, with a tooltip saying "clicking will open a window for it".
...
Any action that takes seconds and changes the world needs both a visible "in progress" state and a concurrency guard that
does not depend on the UI. Only the former, and a second entry point still double-runs it; only the latter, and the user
still clicks repeatedly for lack of feedback. The "restore your last workspace" row got three things wrong at once, each
causing the next:

- Too light: at 11pt / fg2 / no background it was the weakest element on the whole panel, and the reason you are looking at
  the panel is that there is nothing above it. "We haven't counted the sessions yet" is a reason to word it carefully; it is
  no reason to whisper the only action on screen. It now carries the same weight as the "N sessions known" version.
- Nothing changed on click: restoring opens a pile of tabs and waits on the terminal for each, which can take several
  ...
  "正在恢复工作现场…" (restoring…) + a spinner, and the row stops being clickable: that is the honest form of "clicking again
  does nothing"; a button that quietly swallows the click says the opposite.
...
- The working loading ring rotates slowly, one turn per 2 seconds, linear (revised 2026-08-13; it was specified static before). Reason: of the four
  statuses, only working is a process; the other three are conditions at a moment, working is
  "happening now". A static ring makes the shape do the decoding; a rotating one says it outright. One turn per 2 seconds
  reads as "alive" rather than "urgent": urgency is red's job, and **red never moves**.
  - It rotates only in an interface the user has open: the popover, the phone app, the web page. The menu-bar status
    item icon stays static: it is on screen all day, and motion there is ambient noise. (The server-mode
  ...
  - "Only rotates while open" is guaranteed by swapping the rotating view out for a static one whenever the interface is
    hidden; a comment saying so guarantees nothing (measured 2026-08-15).
    ...
    Measured (same build, same fleet, one working agent): 0.1 to 0.3% CPU before opening, 4.6 to 11% after open-and-close and
    still climbing; the commander's menu bar after 3.5 hours sat at 20%, 87.9% with the interface open.
    A visibility gate does nothing: with one added and the state genuinely flipping to false, CPU did
    not move; a running repeatForever does not stop because its driving state changed or its animation was swapped for nil.
    The only thing that ends the animation is the view ceasing to exist.
    Tearing down the whole view tree is the wrong way to get there (tried 2026-08-16): CPU did drop to 0.0 to 0.2%, but every reopen laid
    ...
    interface is not visible; no view, no animation, while the rest of the panel stays alive and measured. Measured 1.0 to 1.3%
    and no longer climbing, the view tree built once.
  - The panel's size has exactly one authority, `NSHostingController.sizingOptions = [.preferredContentSize]` (measured 2026-08-16).
    There used to be two: SwiftUI measured the panel height
    ...
    anybody.) The SwiftUI side keeps
    ...
    on screen this inconsistency shows only as a blank panel, which is why it went unnoticed for two releases.
  - Live Activity (lock screen / Dynamic Island) cannot rotate; that is a platform limit. Tested on a real device
    ...
    The lock screen still carries the signal: the elapsed time at the right of each row is `Text(style: .relative)`,
...
- Still forbidden: page-level spinning loaders (`BrandLoader` therefore pulses instead of spinning; it means "the page is
  loading", and "an agent is working" is a different message, so the two must not share a motion language).
...
- Three tiers of English casing, one rule across all three surfaces (decided 2026-09-05; the authoritative version is MOBILE §6):
  ...
  `read-only`); they are the status language shared by all three surfaces, and a status word takes no name's capital. ③ Small all-caps for the key
  next to a value (`WAITING ON YOU` / `NEWEST`), produced by `.uppercased()` from lowercase source, so the capitals stay a
  visual tier and the source never spells them. Verbs in the keyboard hint bar (`↑↓ select`) are captions and ① does not apply.
...
"gtmux is handing you a one-click action" shape as the update bar. ② Plan fetched and empty (fresh install) → the row does
not appear at all; the empty-state card's tmux guidance is the only thing that belongs there. ③ Plan unknown (not yet polled / the CLI
call failed) → keep the old quiet row; unknown and none are different answers, and quietly removing a usable entry point because one
shell-out failed is the worse mistake.
The update bar's cyan is deliberately left alone: the two render vertically adjacent in the footer, two identical
prominent bars pointing at different actions invite mis-clicks, and cyan means `working` in the status language anyway.
...
- The browser window aligns with the phone's version of this screen (2026-08-12): session groups are collapsible (chevron + name, choice remembered),
  ...
  (Claude 2.x's `pane_current_command` is a version number, #659). Plain rows need a name:
  ...
  Naming order follows "what this step says beyond the previous one": `title` (someone chose it deliberately; a whole path does not count as a name, many shells
  ...
  because the two rule sets were once written separately, and that is when they diverged.
- Identity uses tmux's stable ids (tmux-id-surface, 2026-08-14): what a row is recognised by is the foundation of this screen.
  ...
  `@id` is the anchor); a pane uses `%id + gtmux's own derived label` (`pane_title` is unusable here; measured across a whole fleet, every plain shell pane's
  `pane_title` was the same hostname).
  On each form factor:
  - `%N` leads the row, and is never right-aligned or trailing. It is the row's only stable and unique token (the tab title, `gtmux focus %N`, and what HQ says
  ...
  - Every session draws its window bands (`@id name`, faint background), including single-window sessions.
    ...
    without looking at the tree. A single-window session then put its panes directly under the session header, and that indent elsewhere means "window",
  ...
  - Honest limit: tmux ids are stable only within one server's lifetime (`kill-server`/a reboot renumbers), so they anchor
...
- The browser window's height follows the session count (2026-08-12): it used to open at 620pt whether the machine had 3 sessions or 81.
  ...
  Width 480 is written as a minimum width: `NSHostingController` sizes the window by the minimum, and idealWidth measurably
  ...
  resizable and belongs to the user, so this is "how big to open", with no cap enforced during use; once the user drags an edge (`didEndLiveResize`) it yields permanently.
...
Positioning: gtmux remains a "coding agent command center"; "any pane reachable" is a capability, and the headline stays the command center. The story must not become "a general tmux manager".
...
- The two windows open from two small marks at the right end of the role banner (`态势板` · `知识库`, situation board · knowledge base, `HQReader.swift`, each opening its own window). Icons only, no text; the names stay in the tooltip and accessibility label: they are entries and not controls, and the banner row exists anyway. Earlier they were a pair of full-width buttons under the card that spent the most expensive full row of the popover on two secondary entries (the most expensive pixels cannot be sold to something one step away). When HQ isn't running, that row is still the original explanatory sentence. This is a deliberate revision of the early "clicking only jumps to the pane, no separate panel" rule; the boundary now runs between reading and driving, no longer between screens:
  ...
  - The knowledge window may carry judgement-type actions (menubar-kb-actions, replacing the early "land / retire stay out of the menu bar"): the four verbs `promote` / `land` / `retire` / `dismiss` are all judgements on something already written, and the cheapest place to judge is where you read it. After hq-knowledge-engine there are three more exit-type actions, still judgements, no authoring: `promote` first asks who this one is for (HQ / this machine / the repo / everyone, single choice); entries awaiting take-away get an exit per reader: HQ / this machine / the repo are "write it in" (gtmux moves it for you, `land` without a ref), everyone is "feed back to gtmux ↗" (opens a pre-filled issue, then `land` with the link); `withdraw` takes back a promotion not worth moving. The entry detail gains a three-axis line (kind · source×count · reader), and candidate rows carry a "≈ same thing" family number, matching one `add --capture k1,k2,…`. **Authoring actions stay out of the menu bar**: `add` / `supersede` need a body typed in, and a body typed into a popover fills the knowledge base with entries nobody wants to read sooner or later; that is the very reason the phone's gate opens only two verbs. So what moved this time is the "read / write" line, redrawn as "judge / author"; driving the fleet is untouched, and the red line stays where it was.
  ...
  - The situation board is rendered (`Markdown.swift` parsing + `MarkdownBlocks` rendering). The first version dumped raw markdown source and made the reader parse it in their head, while the phone rendered from day one: one document, two screens, two looks. The block model copies the phone's (`mobileapp/src/ui/markdown.ts`): heading / paragraph / list / code / quote / table / rule, inline only `code` and bold (measured, that is all this board uses: 1 `#`, 2 `##`, 72 `###`, one 6-column table, 50 list items, 254 lines with inline code, 45 with bold).
    - Tables render row by row as cards: that table's 6 columns are all prose, and no window width turns it into a readable grid; the same judgement and the same measurement as the phone.
  ...
  - A "memory" line at the top of the window: how big, how many local snapshots, and whether anything carries it off this disk;
    that last sentence is the CLI's own words (`gtmux hq --records --json`), no paraphrase, so two screens can't say different things about one fact.
    ...
    The verb is export; "backup" would imply a service gtmux does not run.
  - The knowledge base is grouped by topic and collapsible (`knowledgeTopics`, sorted like the phone: topics with more entries first, empty topics not listed).
    On a real machine this base is 386 entries in 7 topics, two of them 177 and 162 apiece; a flat 386 rows is something you scroll through looking for the end.
    Collapsed, the whole base is 7 rows. A short "recent" block stays at the top,
    because "what did it just learn" is the first question on opening this page, and a mis-remembered lesson is never inert: it is replayed into
    ...
    the menu bar expands in place: the same information, interaction chosen per screen.
...
- Popover height grows with the fleet (2026-08-11, corrected 2026-08-12): list height = measured content height, capped at
  ...
  - An open panel that changes size does not re-anchor: measured with 300 sessions, the window landed at y=-3504 on first open and was only
    ...
    this is a repair, never a routine step, because the panel can change height on every poll, and every reopen resets the selection the user is
...
- Judgement in the core, wording at the edge (hq-verdict-single-source, 2026-08-11): the six-state verdict is decided by the Go core
  ...
  one verdict, and looking alike is no part of it. Don't "fix" that difference in passing.
- The command deck is on the phone / web (the menu-bar card only jumps to the pane). Its structure is defined by MOBILE.md §17 / WEB.md §10; don't copy the details here. Copying once caused drift: early mobile did have a "fleet situation board (`/api/digest`)" zone, which `hq-command-page` deleted (the list belongs to the radar; the HQ page answers only what the radar can't: judgement / your call / activity / conversation). The menu-bar side does not change.
- **The command deck talks only to HQ**, and HQ drives the fleet (`send/spawn`); it advises, it never decides for you. To reach a worker's Detail, go through "open session" on a "your call" decision card (the long-press on a fleet row is gone; that fleet-row list was deleted with `hq-command-page`).
...
- **"Here", not "this Mac" (2026-09-23).** The window belongs to the machine the user is sitting at, so its measurement line reads "measured here, just now" and the access heading asks how a phone reaches *here*. The phone cannot use that word — it points at a machine that is not in the room — so it names the Mac instead; both surfaces otherwise say the same sentences in the same order (`MOBILE.md`).
- **Four rules the pairing window follows (2026-09-23, second pass).** (1) **The row you are on reads loudest.** It takes no clicks, because you are already there, but it is marked with a check and a tint, never dimmed: `.disabled` greyed it out, so the most important row became the faintest one. (2) **Every control fills the width.** The window is a fixed 340, so its blocks take exact widths; a segmented picker given only a maxWidth keeps its intrinsic size and centres inside it, which is what left two narrow controls floating over left-aligned headings. (3) **A measurement says when it was taken, and can be taken again.** The round trips carry "measured from this Mac, just now / 12s ago" and a "measure again" next to them; before this they were frozen at whenever the window opened, with no way to ask for more. (4) **A countdown, not a constant.** "The code expires in 5 minutes" is the same sentence one second before it dies; the footer counts down in m:ss and says the code renews itself, which is what happens while the window is open.
- **The pairing window, rebuilt 2026-09-23 (`direct-server-choice`).** It had become a stack: two segmented controls of different widths, a server list, a 220pt code, then five centred lines of different sizes (what to do, the address, a state, a grey note, a link). Nothing said which of them was the answer. Now it reads top to bottom, each block under its own small heading: **how your phone reaches this Mac** (the three-way control), **route** (Standard/Direct AND, under a hairline in the SAME card, the servers Direct can take — a server belongs to a route, and two headed controls read as two unrelated switches), then the code. The code card is the QR at 148pt, one line saying what to do with it, then **the state first and the address second**: the state is what someone opens this window to learn, the address is what they use once it is good. Under the state, one line of consequence where there is one (only on this Wi-Fi / reachable from any network / pick another server to recover). The footer carries how long the code lasts and the way to mint a new one. Off shows neither placeholder nor icon: its one job is to help pick, so it says what each way means. A server row has NO chevron, because tapping it switches rather than opening a page, and **after a switch the window follows immediately** — it used to keep the old address and its "can't reach it yet" until the next poll, which is the same fact with the wrong conclusion drawn from it; for a short window after a move the state reads "just moved to X, reconnecting" and says that phones which have connected before follow on their own.
- **Which Direct server (2026-09-23, `direct-server-choice`).** Direct is a POOL, not one server, so the Anywhere section lists the servers this Mac may use whenever Direct is the backend: each row is a place (its label in the reader's language, its region under it), a round trip measured from this Mac just now, and a dot that says one thing only — whether that server answered. The one in use is marked in WORDS ("in use" / 「正在使用」), never by colour, and a server that did not answer says "no answer" instead of a time, because that is why the phone cannot reach this Mac and the reason to move. **No address appears in the list**: an address belongs to one server, so it belongs to that server, not to a list of places. The list comes from the CLI at run time (`gtmux tunnel --servers --json`), so a server the operator adds shows up with no new version of the app. Choosing another server asks first, and the question names what it costs: a phone that has connected before follows on its own, a device that only ever scanned has to scan again, and guest links made before the move stop working. The pairing code on screen is re-minted afterwards, since it carried the old address.
...
- Two sheets: PairDeviceSheet = one one-time code over three media, which the sheet keeps redeemable while it is open: each code expires after 5 minutes, works once and lives only in the serve's memory, so it is replaced a minute before expiry, after a device enrolls, and when `/api/health`'s `boot` changes (a serve restart); the QR changes only then, and the replaced code keeps working until its own expiry. The three media: the phone scans a QR / the browser opens `url/#c=code` / the terminal runs `gtmux attach`; the ⚙︎ menu's "Pair a device…" and the empty-state CTA go there directly (skipping Preferences). The sheet always shows an access status bar at the top (mode + address + switch); when it detects remote access is off it runs a pre-step first (choose LAN/Anywhere, and under Anywhere optionally the tunnel backend Standard/Direct, Direct greyed out until unlocked → clicking "Enable" only opens the door, the pairing code is produced by ②), self-repairing without bouncing back: Preferences is the management surface, the sheet is the task flow. The status bar includes the backend (e.g. "任意网络 · 标准", Anywhere · Standard). NewShareSheet = name + per-session "visible/input" checkboxes, one step to build the link (input greyed out until visible is checked; at least 1 visible to create); after creation it flips to the delivery page, the same shape as pairing. **The delivery card (rebuilt 2026-09-21): the LINK is the headline and the three media are three equal cards under it.** It used to be a 168pt QR with the browser URL, the terminal one-liner and a read-out line crammed into the 240pt column beside it; at that width the last of them could not fit and what got cut was the code itself (「这个UI都展示不全」), and the rows read as a list rather than as alternatives (「看不出来是三种并列的不同的方式」). So the link spans the full width, wraps rather than truncates, and carries its own copy button; under it sit Phone (the QR) · Browser (copy link) · Terminal (copy command), equal cards each captioned with its MEDIUM, so the three read as a choice. The sheet is 560 wide, because the link is what must never be shown in part. **Nothing on the card says how to hand it over.** A row reading "read out these two lines" was instructing the owner in their own delivery (「用户自己选择如何传递信息即可，read it out这种指令很蠢」); the link carries its own short code, and what to do with it is the owner's call. The three entries share one link and one scope.

**The export sheet (rebuilt 2026-09-21).** A field's label sits ABOVE it, not beside it. The label used to live in a fixed 64pt
box: "Passphrase" does not fit that at 12pt and rendered as "Passphras / e", and no single width could have held both it and 口令
— a side label cannot serve a ten-letter English word and a two-character Chinese one at once. Above the field, the width question
stops existing. The sheet is 480 wide so the primary button finishes its sentence ("Choose where and export…" read as
"Choose where and exp…" at 440). Seven flat elements became three blocks with a rule between them: what this is · the passphrase ·
what happens after. `Show` rides the passphrase field's own label line, where it belongs, rather than sharing a row with the
strength reading, which is a different subject. The strength line stands alone under both fields and carries a MARK as well as a
colour, so the reading survives a colour-blind reader and a screenshot; it is always rendered, empty text and all, because an empty
...
**The indicator sits on the existing logo; there is never a second status item** (corrected 2026-07-31: the earlier implementation added a separate
`NSStatusItem`, so the menu bar showed two gtmux icons, which reads as two apps installed):
...
- Why this red dot is allowed (decided 2026-07-31): the recording indicator is the one visual language every user already reads:
  ...
  Three constraints box the exception in, so it cannot be misread as "an agent is waiting on you":
...
- The popover header echoes it in one phrase: after the summary line, `· 合盖不睡` (stay awake with the lid closed), only while on, red when the guardrail trips.
  Read-only: a notice where the eye lands, with no control attached.
...
`ServerModeConfirmView`). An `NSAlert` wraps too much when narrow, and once an `accessoryView` widens it the system switches to a centred layout, so the title is centred
...
use a hanging indent, wrapping aligned with the body instead of falling back to the left margin.
...
The rule: no SF Symbol in the menu-bar app is declared below 12pt, and no icon is smaller than the text beside it; `check-design.sh` enforces the floor.
Why it became a machine check: the magnifier in every pane-browser search field, the rows
...
systemic: hands writing SwiftUI reach for a small number by reflex. So it is a rule plus a machine check, no longer left to discipline.

Rules (apply to every SF Symbol in the menu-bar app)

1. Icons are always SF Symbols; text characters are never used as icons (`⌕` `✕` `⚙` `▸` and the like).
...
4. A clickable icon's hit area is ≥ 22×22 (`.frame(width:height:)` + `.contentShape(Rectangle())`),
  ...
  - The marker is a prefix, so matching must tolerate a prefix (fixed in v0.51.1, learned the hard way): `gtmux focus` finds the terminal tab by
    ...
    doesn't jump", and only waiting rows broke (only they are marked), exactly the ones the user most wants to click. The fix goes beyond `●`:
...
5. Exemption: the status badge (`StatusBadge`). It is a shape we draw ourselves, its size dictated by §1's status-language table
   (the inline 9 to 11pt is deliberate, because it always sits against a larger identity avatar and is no standalone icon).
