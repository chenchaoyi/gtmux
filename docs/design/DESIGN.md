...
- **无状态角标**（v2）：HQ 恒在监视，无 idle/working 之分——头像不带角标，并删 agent 名/时长等 session 语言。名字旁改挂**舰队光点条**（每 worker 一枚色+形 pip，方=等输入，即微缩雷达）；副题只说值得你知道的事（"api 在等你拍板 · 其余正常"）。HQ 自身等你拍板时**整卡琥珀描边 + 副题红字**（不用角标），状态栏计数仍含 HQ。
