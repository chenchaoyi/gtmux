...
- **就在原来的 brand mark 上**：awake 时在 mark **下方**加一条短横线（约 62% 宽、1.5pt、圆头），
  像机器立着的**底座**；mark 整体上抬 2pt 让位，**图标尺寸不变**，开关时栏宽不跳。
  一个 gtmux 存在，只是状态不同；两个图标 = 两个 app。
  （2026-07-31 二次修正：先前画的是**环绕四周的细框**，18pt 下就是个白方框 —— 既不表达 awake、
  又像渲染瑕疵。底座线更小、更安静，而且真的有含义。）
- **环是中性色**，不用状态色 —— 颜色只表达舰队里最紧急的 **agent** 状态（§9），服务器模式是
  **机器**状态。**形状承载这个信号**，正是设计语言本来要求的。
- **开着才有，关掉即消失**；**零动效**（§10 全产品只允许 idle→waiting 一次脉冲）。
...
"服务器模式是什么"（一句话，不占常驻版面）；开启前的说明卡用**固定宽度 420pt 的
`accessoryView`** 撑开 —— `NSAlert` 默认列宽很窄，同样的文字会被折成一条又瘦又长的带子。
它是**自己的分组**，排在
