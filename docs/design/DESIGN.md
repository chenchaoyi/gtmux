...
- **卡片下带两个只读阅读面**（`态势板` / `知识库`，`HQReader.swift`，各开一扇窗）。这是对早期「点了只跳 pane、不另开面板」的**有意修订**，边界重画在**读 / 驱动**之间而不是**屏幕**之间：
  - 原规矩约束的是**驱动**（派活、发消息、拍板），那需要输入面，留在手机/网页不变 —— 这两个面**一个动作都不提供**。
  - 而**读**恰恰在 Mac 前最该发生：态势板描述的就是这台机器，读者人已经坐在它前面。
  - **知识库的落地 / 退休不放进菜单栏**：那是写操作，`gtmux knowledge land …` 就在手边。红线因此仍在原处 —— 菜单栏能看 HQ 的记忆，不能通过 HQ 驱动舰队。
  ...
  - 数据仍**只经 CLI**（`gtmux hq --board --json`、`gtmux knowledge list --json`）—— 菜单栏不认 HQ home 在哪，那个路径可重定位、且本机是符号链接。
