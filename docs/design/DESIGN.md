...
| 项 | 值 |
| --- | --- |
| popover 宽 | **420**（对标 MPBar；长内容单行尾截断，不靠加宽换行） |
...
2. **HQ 卡（中控，hq-presentation）**：header 与分组列表之间的常驻层。中控是元层会话
   （参谋长 / 看管者），**绝不**混进分区行，也**绝不**长得像一行普通 session。两个区分手段：
   （a）**角色横幅**——卡片上方一行小号全大写标签「参谋长 / CHIEF OF STAFF」（望远镜图标）
   + 右侧一句用途「统观全局 / watches all sessions」，一眼点明这是统观层而非一个会话；
   （b）**带边框的面板**——运行中的卡片是一个 1px 描边的圆角面板（agent 行**无**边框，这是最强
   的「不是一行」信号），内含品牌网格标（§12，非 agent 头像）+ 状态徽章 + task 一行 + 相对时间，
   点击跳转。chrome 保持中性（守铁律：色彩只表状态，卡上唯一的状态色是徽章）。未运行 = 虚线
   幽灵位「中控未运行 · 点击启动」（shell `gtmux hq`）。搜索态与真空态下隐藏。
...
**分组 Form（macOS 系统设置原生范式）—— 不再用扁平栅格。** 用 `Form` +
`Section`，`.formStyle(.grouped)`：每组一张带分区标题的卡片，组内每行一个设置
（label 在左、控件在右；多选项用下拉 `Picker`，布尔用 `Toggle`）。组织上**对标 Moshi**
（与移动端 §6 同构），让设置一眼可扫。窗口约 460×640。分组与字段：

- **通用 General**
  - **语言 Language**：下拉 `跟随系统`（默认）/ `English` / `中文`。读取 `GTMUX_LANG`，
    锁定后写入偏好并覆盖；切换**即时生效**，状态项、popover、通知全部跟随。
  - **开机自启**：开关。
  - **全局热键**：⌘⌥G（打开 popover）。
- **状态栏 Status bar**
  - **显示**：下拉 `点+数字` / `仅圆点` / `空闲时隐藏`（对应 §2 三种模式）。
  - **刷新间隔**：滑块，默认 1.5s。
- **通知 Notifications**：开关（agent 开始等你 / 完成时提醒）。
- **远程访问 Remote access**（**独立区，是 pair 与 share 共享的「门」**——两者都从这里进来，
  故不嵌在「我的设备/Pair」里）：分段 `关闭 / 局域网 / 任意网络`（合并 serve+tunnel 的单一
  选择器；`任意网络` 为 Pro 功能，未解锁弹 paywall，解锁后弹常驻敞口确认），下附一行
  状态副标题（见 `Preferences.swift` 的 `remoteModeBinding` / `remoteSubtitle`）。
  **任意网络开启时再露一行隧道后端**：`Standard`（零配置托管）vs `Direct`（自建 VPS+域名）；
  本机已配置 Direct 时给 `Standard | Direct` 分段切换，否则只读标注当前后端 + 如何配 Direct
  —— 别把 `gtmux tunnel --backend` 已有的选择藏在含糊的「任意网络」后面（见 `tunnelBackendRow`）。
  隧道后端同时决定 pair 与 share 链接的可达地址（同一条隧道），故归门区、不归 Pair/Share。
- **我的设备 · Pair**：从门进来的、你自己的设备（手机/浏览器/终端，全权），列表 + 逐条吊销 +
  「配对新设备」（见 §PairDeviceSheet：一码三介质）。
- **分享 Sharing**：协作者最小授权（同意开关 + 每链接 See/Type + 复制/吊销）。
- **软件更新 Software update**：当前版本 + 状态（已是最新 / 有可用更新 → 立即更新）收成一行，
  重新检查是行尾一个轻量 ⟳；只有真有可更新/失败才单独强调成带按钮的一行（见 `updateRow`）。（同样的分组语言 + value/chevron 抽屉 PickerSheet）。两块屏
> 共用同一套设置组织：分区卡片、行内控件、多选项收成「当前值 + 抽屉」。
