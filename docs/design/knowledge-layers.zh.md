...
`README.md` 当索引，而台账里指向同一批脚本的条目有三十处 —— 一个问题两套索引，派活前没人读的正是那一套。
`gtmux knowledge lint` 会报没有条目指向的脚本（`orphan-tool`）和指向不存在脚本的条目（`broken-tool`）。
家目录顶层就是那四样，没有别的：设计简报是 `notes/` 里的一篇笔记，不是一个 `designs/` 文件夹。
...
打上语言标。本机的文件（主题文件、`machine.md`、各 agent 的索引块、仓库块）按台账的多数语言渲染，不跟渲染
进程走（launchd 起的 serve 没设 `GTMUX_LANG`，跟进程走会让每个 agent 的指令文件来回翻）；`everyone` 的简报和
issue 用英文。缺一半只是 lint 里一条 `monolingual` 计数，HQ 分批补，绝不因此拦住一次记录。change
...
却没打标的条目 —— 那就是没问就写的。别人的密钥照旧不进库：只记它在哪，不记本身。盘上不加密；会离开机器的
