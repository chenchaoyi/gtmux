...
1. **身份，永远第一块。** `%N · session:window.pane`，然后项目 · 分支。**`%N` 打头** —— 跟
   授权挑 pane 那条同一个理由：名字只是注解，id 才是锚，两个 pane 跑同一个项目，行上截断后
   长得一模一样。
2. **它在干什么。** 完整任务（不截断）、完整错误原文（⚠ 琥珀）、后台活儿（⧗），
   状态 + 时长用状态语言的词。
...
4. **手感**：弹簧入场（不是线性滑入）+ 按压反馈。**触觉反馈还没有** —— 需要一个原生模块，
   单列。按下去毫无回应，是「没手感」的主要来源。
5. **别的都不放。** **不放不可撤销的操作** —— 长按只负责**打开**这张卡；真正动手要在卡里
