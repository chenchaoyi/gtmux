...
**照片直接进暂存，不经过标注编辑器。** 以前每张照片都被推进标注器，等于给普通情况垫了第四块
全屏界面，出来的文件还一律叫 `markup.png`，画没画过都一样。现在标注挂在**缩略图上**：点一下
你刚加进来的那张图就进编辑器，改完**替换原件**（追加会把同一张图发两次）。粘贴仍然先进编辑器，
这一条和界面无关 —— 剪贴板给的是 data: URI，编辑器是把它落成文件的那一步。
