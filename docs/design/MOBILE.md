...
- **列表要说出自己在哪儿结束**（2026-09-05）。它原来停在最后一行、下面留一片空，读起来像
  「还在加载」而不是「就这些了」。末尾加一行居中的淡字 `到底了` / `end of list`。
  **平时不报总数**：这张列表的总数不是舰队的总数（参谋长在浮窗圆盘上，不在分区里），
  footer 写「16 个」而顶栏写「17 个」，读者会去找那个跑掉的。**只有分区被折叠时才补一句
  `显示 5 / 16`** —— 那时候「就这些了」本身就不成立，一个撑不住的断言比不说话更糟。
