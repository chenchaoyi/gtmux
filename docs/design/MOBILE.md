...
2. **判断（⟣ 常驻两行内）** — 确定性结论（与 HQ 卡片 `fleetHeadline` 同一套规则，故卡片与页面
   ...
   （`GET /api/hq/board`，只读全屏 page sheet）。**新鲜度必须显示**——态势板是人维护的综合判断，
   ...
   **态势板阅读器（sheet）的规矩** —— 它是一份**长文档**（写这段时真板 842 行），不是一张卡：
   - **按 `##` 分节、可折叠**，顺序**永远是作者的顺序**，不排序（HQ 会把「先读这段」钉在最上，
     排序会把它说成最旧的一条）。折叠头右侧带**计数气泡**（跟雷达分区头同一个语汇；一个光秃秃
     的数字贴在右边缘不说明它在数什么）。
   - **文档自己的 `# 标题` 不渲染** —— sheet 头部已经写着「态势板」，再来一个更大的标题是重复。
     副标题只说**多久前更新 · 只读**，不再重复一遍名字（曾经是「Situation board / situation
     board · 50m ago」）。
   - **行内 code 在这里不套底色块**（`calmEmphasis`）。聊天是深色面、code 少见，色块正好；
     而板子一行好几个 span，浅色页面上就成了一片白色荧光笔痕 —— 更糟的是 span 落在折行处时，
     用来充当内边距的那个前导空格会带着底色渲染成一个**悬空的白方块**（iOS 不认嵌套 `<Text>`
     的 padding，所以当初用空格顶）。等宽字形本身已经足够说明「这是个字面量」。
   - **超过 3 列的表格改成逐行堆叠**（`stackRows`）：手机宽 ~390pt，板子那张 7 列表只画得下
     三列半、第四列拦腰截断 —— 那不像「可以横滑」，像坏了。堆叠后每个值都带列名、都看得见，
     空值和 `—` 直接不画（半数格子是空的，一个后面没有内容的标签只是噪音）。
   - **文案由 gtmux 给词，不由 HQ 现译**：板子的两节固定叫 `① 现状` / `② 交接记录`，列名固定
     `pane / 在做什么 / 谁派的 / 优先级 / 状态 / 等你定 / 教训`（playbook v22 起写死）。此前
     playbook 只用英文命名（Posture、一行一个 "ship"），HQ 自己译出了 `① 姿态 — 当前在飞的线`
     和列名 `线` —— 逐字对译，中文里没人这么说。**给词，别让它翻译。**
