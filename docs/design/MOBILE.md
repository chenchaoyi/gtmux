...
The full iOS size set 20 to 1024 (@2x/@3x): notification 40, settings 29, spotlight 20, home 60,
...
- 34pt, a rounded square with radius 9 (a rounded square rather than a circle, because the rounded square signals "an app icon goes here"),
...
Every status section header is a tappable collapse bar. Discoverability is a hard requirement, so a small chevron on its own is not enough:
...
(dark `rgba(255,255,255,0.16)` / light `rgba(0,0,0,0.16)`). "Needs you / working / idle" then read as separate blocks at a glance instead of one continuous column.
...
- The two top-bar buttons each own a 40pt square and never overlap (2026-08-16). They used to be "14pt between the icons + a 10pt hitSlop on every side of each", so the two hit areas overlapped by 6pt in the middle, and the overlap belongs to whichever renders later: aiming at the right half of "all panes" opened Settings. Adjacent buttons are judged by their hit areas; the visible spacing between icons says nothing about where a tap lands.
- The list for picking a pane for a link has the same shape as the pane browser: grouped by session, every row led by `%N`, then the task name (2026-08-16). It was a flat list of task names, so two panes running the same project in two different sessions looked identical once truncated, and the user had nothing to tell which one they were authorising. An authorisation screen least of all can run on "probably this one": the id is the anchor and the name only annotates it, and that matters more here than anywhere.
...
1. The header is the anchor, one line. `session · %N`, with status + duration + project·branch on the line below.
   ...
   Identity no longer gets its own colour block either; that read like a system diagnostic rather than something you can act on.
...
4. The actions are a group of icon-led list rows (2026-09-03) in one rounded container with hairline dividers; four separately bordered cards would compete with each other instead of reading as a menu.
   ...
   - Every action must say exactly what it sends. "Continue" is a label, and a label is no contract; the subtitle reads
...
Four kinds of row each tell their own truth (the easiest place in this block to get wrong): the radar holds agents inside tmux, sessions sensed outside tmux, and ordinary panes the user pinned. They used to pop up identically, and for the latter two the result was wrong rather than merely abbreviated:
...
- A watched ordinary pane: give `%N` and its command, and no agent status; it has none, and writing "idle" is inventing a fact.
...
- The row begins with `%N` (the pane's tmux id) rather than the shifting `w.p` coordinates. Tapping it copies `gtmux focus %N`
...
- Window bands (`@id name`) are inserted for every session, including single-window ones (reversed 2026-08-14: the old "save a row" was counting rows instead of reading the tree; a single-window session put its panes directly under the session header, and the indentation meant something different from everywhere else). The session header always lists all its window ids (more than 5 fold into `+N`); a folded session must still say what it contains.
...
- Indentation must actually indent: pane rows sit inside a window band, so their left margin must be larger than the band's (26 vs 22).
  It was once the other way round, rows at 14 and bands at 22, the child further left than the parent, so on the phone it read as a flat list while the menu bar was a tree, and the two screens did not look like one product. The band's ground is a faint value of the text colour (neutral grey at 10%) rather than `surface`: in the light theme `surface` is pure white and the page is `#F2F2F7`, so the band would be brighter than the content it groups, the opposite of a ground tint. A 7pt gap sits above the band and outside its ground; it is separating two windows.
...
A long-press on a radar row gives feedback on three layers: a haptic at the moment of recognition, motion in the row while it is held, and a menu that grows out of the row. The card it opens (`RowSheet`) used to give no feedback at all: the row faded to 0.6, and 350ms later a menu sprang up from off-screen.
...
A sheet's containers may not be accessibility elements: the layer that only swallows taps is a `View` + `onStartShouldSetResponder`, and the tap-outside-to-close layer keeps its Touchable with `accessible={false}`.
...
A Touchable is an accessibility element by default, and iOS collapses an element's whole subtree into it, so VoiceOver could reach only "this card": no action inside it, and no body text either. It took a real press through Appium to notice: a driven long-press could not find a single action row.

- The inner one exists only to swallow taps; it became a `View` + `onStartShouldSetResponder`, which is no element.
...
(send Esc, stop the running turn) looked identical to "View changes".
...
- "Interrupt it" uses amber (`ERRORED_COLOR`, with the darker `#B45309` for text so it reads as a label on white rather than a warning badge).
  **Never red**: in this product red means only the "waiting on you" state, and using it as a button colour would collide with the status language.
- The header says one sentence. It was "waiting · 4 min", a status word and a duration joined by a middle dot;
...
### Collapsing the top chrome: chrome is an overlay (redone 2026-09-10)

The top chrome (the header + the neighbour bar + the controls row, the segmented control and the tool keys on one line, see below) is an overlay: it floats above the scroll view, slides out with `translateY` when collapsed, and the scroll view's frame never changes. One driver folds all of it away while reading history to give the height back to content, and brings it back at the live tail; the content carries a constant top inset (`topPad` = the chrome's height), so the oldest line can still slide out from under the chrome.

The mechanism took three versions to get right, and every defect in those three versions came from the same decision: collapsing by *animating the chrome's height*, so the scroll viewport changed with it.
...
The judging rules were sound in every version, so the fix had to be in the geometry, and the chrome now does not participate in layout. With the geometry constant there is nothing to compensate, nothing to fight and no loop; `gap` means the same thing before and after a collapse.
...
- Child views report a distance, never a boolean; the host compares it against the thresholds.
...
Detail's top is three bands: the title, the neighbour-pane bar, and one controls row with the segmented control on the left and the tool keys on the right. That is 115pt, 15% of the usable screen height, about 5 more terminal rows than before.

It used to be four bands: the title, the neighbour-pane bar, a full-width `Chat | Terminal`, and a tool row
...
Both the phone and the Mac lift the board's "still waiting on your call" section to the very top, in waiting red, and draw nothing when it is empty.

It used to be a `###` under `## ① 现状` (current state), visible only after expanding the section above, and it is the only part of the whole board that belongs to the commander rather than to HQ.

The cost, stated plainly: this requires gtmux to own that heading. Every other heading on the board is a name HQ chose, and a surface can only lift a section it recognises. So the heading is written into `boardSeed` (playbook v36), matched in both the Chinese and English spellings (a board keeps the language it was seeded in); a board matching neither shows no band, an honest degradation rather than a guess.
...
Each item in the commander's section is a row (`boardSections.askItems` reads HQ's numbering and bold group headings; the group stays as a label) with **Tell HQ ›** at the right. Tapping asks the two things a commander says to a chief of staff: "Do as you suggest", offered only when the item carries a recommendation, sends 「态势板「还等你定的」第 3 条（…）：按你的建议办。」 at once; "Let me say…" closes the sheet and puts that quote in the composer with the cursor after it.

The section is a numbered list of decisions only he can make, and HQ writes its recommendation into most of them, yet the sheet was read-only, so answering item 3 meant closing it and typing "about item 3…" into the composer from memory (「没有直接处理告知 hq 的入口」). The quote names the item by HQ's number and first line, so HQ knows what was decided without re-reading. The board's text is untouched; the reply is the ordinary send. The Mac reader stays read-only by DESIGN §12's rule: a reply is driving, and driving lives here.

### The board must say how fresh it is (2026-09-09)

Both ends draw "updated N ago" under the board's title, from one pure function (`boardAgeText`).

The phone's situation board already said "updated 3 hours ago"; the Mac said nothing, yet `updatedAt` had long been in the Swift model, just never drawn. A stale board read as the current state is a failure mode with real cost:
it is HQ's understanding of the fleet, and the whole reason to open it is that the understanding can be trusted.

The banded copy is tested at every band's boundary rather than some convenient point in the middle;
a one-second-off boundary is exactly the kind of thing that looks fine in a screenshot. The Chinese is written on its own terms rather than word for word from the English: Chinese puts the time first.

### The menu bar's knowledge base: list and body side by side (2026-09-09)

The menu bar's knowledge base is a 292pt list on the left with the body on the right; window minimum width 520 → 700, default 640 → 820 (that 520 floor was for a single column).

The window is 640pt wide, yet the content was one column with 12pt padding; reading an entry meant leaving the list and coming back, a screen change for "what does this one say". That round trip is gone; the back control went with it (the list is beside you now), but the body keeps a topic-name line at its top: it says where the entry lives, which the list only implies.
...
Search. The knowledge base is searched by title, id and topic, case-insensitive; whitespace tokenises, and every token must match (two words narrow, they never widen). Until now, with 396 entries in 7 topics, the only way in was "you know which topic it is in", knowledge about the knowledge base rather than about your machine. Ids are searchable because HQ refers to entries by id when it dispatches work; that is what a person will actually paste.

While searching, the index gets out of the way: the index *is* the answer to "browse", and two answers on one screen is confusion. Clearing the input brings the index back. The search box appears only at the index level: inside a topic or an entry the question has already been narrowed.

The four lines of explanation under "waiting for you to take" collapse into "What is this ⌄". They describe the mechanism of promotion itself, worth reading once; left standing they are just height. It opens collapsed every time (by a reset on open rather than an initial value, so last time's open state does not carry over).

Name by action. "Retire this entry…" describes a mechanism; what you are really saying is that this lesson no longer holds, which is precisely what the dialog is about to ask you. Now the button and the question say the same thing.

### The HQ page is a report (2026-09-09)

The HQ page reads as a chief of staff's report: the three document doors are present from the first frame, the verdict sentence carries the weight of a conclusion, the three tabs are pills, and the quiet state says what is running.
...
HQ's three documents are always present. Board / knowledge base / usage used to be GridRows inside a fold, and the fold `briefOpen` defaulted to `false`: open the HQ page and none of the paths to those three showed, with no other entrance on the phone. Now they are a permanent row of three small cards, each carrying its own live value (how fresh the board is, how many entries the knowledge base owes you, how much of the weekly quota). The card that owes you something uses waiting red, judged by the model's own
`owed` row, never by sniffing for digits in a string ("352 entries" is size; treating it as debt would cry wolf in the most ordinary state).

This overturns the old "documents and figures are rows of one grid, sharing a key column": a shared key column made them look like more readings, and a destination you have to lift a lid to discover does not work as a destination.
...
(see "chrome is an overlay" above): one driver, both bands fold together, sliding out with `translateY`,
...
An ordinary pane needs a name, and its command name is the last resort (`api/types.paneLabel`). It used to print `command`, so three shells in one session were all called `bash`: true, but distinguishing nothing, and the reader is choosing one of these chips. The naming order is by "what this step says beyond the last":
...
- Stage 2 · the native layer (`mobileapp/ios/TermSelection/`): a transparent `TermSelectionView`
  ...
  RN Text, so the advance of a CJK fallback is measured rather than assumed to be 2×cell).
...
### Input history is scoped by "work" (2026-09-06)

Input history is scoped by project (falling back to the tmux session name), with this scope first and a cross-scope "recent" topping it up.

Changed only after measuring on a real device: 35 projects, 8791 prompts, filtered to lengths a phone would type.
96% of distinct entries occur in only one project, and they recur within that project
(`发布` 29 times, all in the same project). Only 4% genuinely cross projects, and they are the handful
(`continue` / `继续` / `ok` / `/compact`) that belong to quick replies rather than history.
...
That is what needed fixing; "there are unrelated things in it" was only the symptom.
...
- Delete by text, and from both sides. The list is "this scope + top-up" stitched together; the 4th item is a position in the rendering,
  and no position in storage; deleting from the scope alone lets the top-up push it back one row lower, which reads as the delete not working.
...
- Empty input means no draft rather than an empty draft; otherwise blanks crowd real drafts out of the cap.
...
### Knowledge base: "Recent" is a view
...
Topics can expand in place as well as being entered. Both ends use a fold row (`▸`/`▾`); one tap shows the topic's
...
Backing out of an entry returns you to where you were. Back returns to where you tapped in from (a topic page returns to the topic page, never a blanket jump to the root), and each level remembers its own scroll position and restores it. Previously, tapping the fourth entry of a 164-entry topic and backing out
...
trip back to the input bar in the middle (user feedback 2026-09-07). Closing by tapping the backdrop or the back gesture
...
photo. **Defaults of this kind follow frequency of use**, and a count of steps is the wrong yardstick.
...
The icon expresses the action: exit full screen uses `✕` (close) rather than `⤡` (a diagonal resize arrow reads as
"bigger/smaller"), and is paired with the full word ("Exit full screen") to leave nothing to guess. Superseded on the glyph: §17's full-screen exit control is now four corner marks, and `✕` was rejected there for reading as "close this content"; the opacity rule above still stands.
...
exception-state reminders rather than chrome, and still surface. In terminal full screen the safe-area backing must use the terminal's own dark
...
fixed frame: you read the live tail at the bottom, and the older content in the island's band is one flick away. This differs from the 2026-08-09 case:
...
- iPad 1/2 split (683 wide) and Slide Over (320) are compact, deliberately; 2/3 split (910) is regular.
  **A phone in landscape stays compact**: iPhone landscape at 852 to 932 wide and 393 to 430 tall fails the height line, because the regular shell is an iPad layout and a wide phone is still a phone.
...
- Sidebar: 300pt when ≥ 1000 wide, otherwise 280pt. Its content is the phone's radar: brand + server name + connection dot + one summary line;
  ...
  (a persistent sidebar uses a card rather than the floating disc) and the "all panes" entrance.
- The sidebar can be collapsed (top-left button or ⌃⌘S, remembered). The 2026-07 version said "in portrait or a narrowed split the sidebar becomes a ☰ drawer";
  ...
  that does not exist in the numbers. "I want the whole width" is served by the collapse button, which also covers 768 to 1000 windows in Stage Manager.
...
- Hardware keyboard: one native bridge (`UIKeyCommand`) + one table (`src/keys/keymap.ts`). ↑↓ move the radar selection, ⏎ opens,
  ⌘1 to ⌘9 jump to row n, ⌘⇧H HQ, ⌘⇧P all panes, ⌘F search panes, ⌘K focus the input box, esc closes a sheet, ⌘[ ⌘] chat/terminal,
...
- Orientation: all four supported; none is locked out.
...
- English capitalisation has exactly three registers, decided by "what this text is doing" (settled 2026-09-05; before that
  the three tabs were `your call` / `what HQ did` / `console`, with `Situation board`
  and `Activity` on the same screen, and the user's verdict was 「大小写感觉很随意不专业」, "the capitalisation feels arbitrary and unprofessional"):
  1. Sentence case for every "name" and "action": section/tab titles, buttons,
  ...
  2. All lowercase for the status words themselves: `waiting` / `working` / `idle` / `running` / `errored` /
     `offline` / `reconnecting…` / `read-only` / `landed`. This is the status language shared by three surfaces;
     a status is a state, and capitalising it promotes it to a title.
  3. Small all-caps (eyebrow) for the key beside a value: `FLEET` / `USAGE` / `BOARD` /
...
- The card's ground follows the state (a very faint red / cyan / green / grey gradient). It is a backing only;
...
- The server name is the card's identity. gtmux connects to several Macs and switches between them (`App.tsx` keys on
...
- The loading ring does not rotate, and that is deliberate. Measured on a device 2026-08-13: `.repeatForever` rotation
...
Spanning versions is the core scenario: a user who skipped three versions must see all three versions' notes.
...
- Two tiers, isomorphic with the CLI (superseded by the fold above; kept for the record):
  - The popup = a summary. Grouped by version, newest first, truncated to 8 items (the CLI's `changelogMax` is 5; here it is
    a card the user is actively reading, and 8 lets a common single-version release (5 to 6 items) show in full, with folding appearing only when "you really did
...
- Three rules for truncation (all in the spec): a version shows in full or folds in full ("3 of 0.46.0's 6 items"
  means nothing to a reader); the fold is always a suffix (skipping a middle version to fit a smaller one
...
- Visuals (§6 iron rules): no NEW/ENHANCED/FIXED columns, no emphasis fills, no motion; release notes are
  read once and closed, and dressing them up is the forbidden marketing register. A centred card rather than a full-screen page; the backdrop or the button both close it. Items
  are prose and wrap rather than ellipsise (§8's CJK ellipsis rule is for list rows).
- Brand comes from the product's own vocabulary: the card header holds `BrandMark` (the app icon's pane grid),
  version numbers are always `Menlo` (the same monospace as the terminal, branch chips and HQ figures; a version is a token, and tokens are set in monospace),
  and the bullet is a pane cell (a 5pt rounded square) rather than a typographic dot. Nothing more.
- The version number does not appear twice: when group headings are present, the header's version chip yields; with a single version, the reverse. Printing both only adds noise.
...
Tapping a `role:"supervisor"` row lands on the dedicated HQScreen instead of an ordinary Detail.

**Iron rule: the HQ page never lists the fleet row by row.** That list belongs to the radar, one step back; the old "fleet situation board"
...
1. The permanent header is two lines: `‹ gtmux HQ ● ` + one verdict sentence (turning amber when someone is waiting on you). That is all.
   ...
   - The verdict sentence and what it expands into must be one object (2026-09-05: 「这一条让人感知能点击的感觉太弱了」, "this row barely feels tappable").
     ...
     the arrow the same `›` the whole app uses (rotated 90°), its weight following the sentence it belongs to rather than "the faintest grey available".
     A disclosure control that does not look like the thing it opens gets read as a line of explanation.
   - The quote is the chief of staff's latest sentence, and nothing is quoted when that sentence is routine bookkeeping
     ...
     The level comes from the chief of staff's own register marks
   ...
   - A brief needs a source and a time, and only a glance's worth (byline + body capped at 3 lines + at most 3 `· ` items).
     A passage with no byline and no time is just a passage; and the full text is already in the conversation below, so printing it again in the header
     puts the same words on one screen twice. The time is the turn's timestamp; if unavailable, none is shown, never an invented one.
   ...
   - The chief of staff writes markdown; the header either renders it or strips it, never prints the markup itself (``` `%19` ``` was once shown
     with its backticks). Paired backticks render as monospace; a lone one stays as is, since guessing where it ends would silently restyle the rest of the sentence.
   - Everything in the expansion is a row of one table: a key column, a value column, five rows straight down (2026-09-05:
     ...
     The key column's width is a measured value per language (en 80 / zh 48); five rows sharing one width is what makes it a table.
   ...
   - The usage row compresses to "one window per plan" (2026-09-06).
   ...
   - The expansion is a chief of staff's report (redone 2026-09-06). Measured by the page's own criterion,
   ...
   - So the expansion gives three rows in the order of a chief of staff's report, each key being the question it answers:
     ...
     - Did (did): what the chief of staff did in the last day, taken from the same tally as the "HQ's work" section, so the two cannot disagree.
       This row is the piece the old version lacked entirely, and the reason it read like a dashboard (the week's figures are under "HQ's work" below).
     - Context (context): fleet + usage + machine pressed into one row, shown only when things are out of the ordinary:
   ...
   - The three registers (quotation · figures · document doors) are separated by hairlines rather than spacing.
...
2. Three sections (segmented switch, each filling the whole body, never squeezed onto one screen; in the regular shell the conversation is permanent and the other two sit in the right-hand inspector, §5). The three labels are three peer noun phrases
   (`Your call` / `HQ's work` / `Console` · `该你拍板` / `参谋长动作` / `对话`); a label names a place, so mixing a noun, a question and a noun breaks the set (§6 case rules).
   ...
   - HQ's work: the chief of staff's own actions (`gtmux:audit:*`): dispatch / reap / bookkeeping / self-check /
     ...
     and none of it was visible in any interface, while this section was showing "who started, who finished". That gap is what made the page read as a dashboard: a chief of staff that acts for you has to show what it did.
     ...
     - Wake deliveries do not count as actions: that is gtmux knocking on its door (1532 a week vs 39 actions), and counting them buries the actions.
       Channel failures stay visible, as the `wake-degraded` alert, which is the part worth hearing.
     - The tally order is fixed: ordering by frequency would put the ledger (168) above dispatch (27), which matters more, and rows that reorder whenever
     ...
     - The timeline needs rhythm: in a column of evenly spaced rows, six quiet hours and a two-minute interval look identical
       ...
       asked for a compact screen. So the gap is stated instead of drawn: actions close together collapse into a cluster (threshold 20
       ...
       distance. A thin rail on the left strings the entries of a cluster together and breaks at a gap, so the break itself shows the pause.
   ...
   - Console: the conversation with HQ (ChatView). A clear is a seam (hq-console-history, 2026-09-15: 「每次只能展示上一次 clear 后的一点内容」): HQ starts over often, each start a new session log, so the console showed only what came after the last one. The serve stitches the session before it on request (`?earlier=N`, following the `hq-session` audit chain), the seam reads `— 新一段对话 · 23:03 /clear —`, and 「▴ 载入上一段对话」 sits above the oldest turn while one more is known; each tap is one more hop. A worker's Detail has no chain and offers nothing. The process must be visible while it runs:
...
- Each window's bar takes the anatomy of claude.ai's plan usage page (usage-bar-tiers, 2026-09-17: the commander pointed at it and chose blue from the canvas in `docs/design/mockup/usage-bars/`): 6pt high with round ends, the track a pale wash of the fill's own colour with a hairline edge, a round dot as the least a non-zero value draws, and the figure written `已用 N%` / `N% used`. The colour is the core's `tier` and nothing else: blue (`#4177D0`, `#5E8FE3` in dark) for an ordinary window, amber when the core warns (a weekly window past `limitsWarnPct`, 85% by default; a session never), the status red at 100%, where the agents on that plan stop, with the figure in the same colour. This is a stated exception to "colour encodes status only", on the same ground as the activity greens: it is a chart of Claude's own plan, and it reads the way claude.ai draws it. The page never judges a percentage itself; an older serve without `tier` gets red at 100% and no amber.
...
- Sessions sort by "trouble": those the core alerted on first, then by burn rate, then by context share.
...
- The three machine rows carry resource icons (2026-09-10, overturning "no icons added for looks" above). The icon says only which resource
  (a disk platter / a memory stick / a gauge), and nothing about whether it is fine; status is still triple-encoded as colour + `⚠` + wording, DESIGN §1 still holds,
...
- The door is permanent even though the summary is conditional. The header's "context" row disappears when nothing is moving,
  and by design it is dropped when the machine goes red, exactly when those readings matter most.
  So the door into this page stands on its own rather than hanging on that row.
...
adjacent to the board: working memory and long-term memory side by side). On a real device it is 330 entries / 7 topics; the phone answers three questions in order and does not try to be a browser:
...
1. What does it owe me: promotions waiting to be taken come first, oldest at the top. This is the only step in the whole knowledge lifecycle stuck on a person:
   ...
   Overdue uses the same line as `gtmux doctor` (about two weeks); two places giving different answers about one queue leaves the reader trusting neither.
...
- The row says how big, beyond a mere tick: "backed up" and "2.6 MB of irreproducible notes backed up" are two different sentences.
...
**Rules of the board reader (`BoardSheet.tsx`)**: it is a long document (the real board was 842 lines, 48,000 characters when this was written), so it is read as an outline rather than a card:
...
Segment labels carry their own signals: `该你拍板` (your call) carries an amber count badge, `参谋长做了什么` (what the chief of staff did) carries a dot when there are unread actions (unread fleet events do not light it: lit for something that is not the section's subject, it sends people to the wrong place); the section you are not looking at must
...
The full-screen exit control is an icon: four inward-pointing corner marks (`ExitFullScreenIcon`),
...
The general rule for small icons: remove strokes and keep the size.
No single diagonal arrow (tried and replaced; alone it reads as "resize" rather than "leave"); no `✕` either,
...
Back-to-bottom is "an arrow + a bar" (`ArrowToBottomIcon`): a bare `↓` says
...
`#06B6D4` for the strokes, never as a fill: it pairs with the exit-full-screen control at the top left and stays clear of the status language;
...
and `disk_use_pct` samples the data volume `/System/Volumes/Data` (the read-only `/` would give a capacity % that is badly low and misleading).
...
resource likewise red only at the red tier, soft amber stays amber). The only difference is the container: the phone is a draggable floating disc, the menu bar a card avatar
...
- Demo mode (mockup §18): a full-function demo with no server (the App Review path). Iron rules: visibly sample data (the DEMO chip throughout), never mixed with real data (Servers has no entries), reset on every entry, every step guiding to "Pair your Mac". Demo renders the real shells (since 2026-09-12, SURFACES.md §3): `SplitShell` on iPad, `RadarPanel` on the phone; exit / sample banner / pair button are the panel's `demo` prop; it is no longer allowed a radar of its own (falling behind the device twice was caused by having one). The script's main line = the 30-second core loop: see one waiting → tap in → press 1 to approve → tests finish → the radar turns green with latest. Refinements in ITERATIONS §F7.
  - Demo must be the same as the device radar (settled 2026-08-12, after two failures for this very reason):
  ...
  - Demo-only things stay: the sample banner, the "Pair your Mac" button, the close button; these are demo scaffolding rather than product differences.
...
When the Mac is in server mode (lid closed, not sleeping), the phone must show it at a glance and **offer no switch**.
...
- Presentation = a thin ring around the connection dot: the same colour as the dot (connected green / reconnecting amber / offline red),
  shown only while server mode is on. Same colour by design: it reads as another state of the same indicator, with nothing new to decode. The VoiceOver label appends "server mode on".
- No chip, no list row, nothing in the radar: server mode is a machine state, and the radar lists agents.
