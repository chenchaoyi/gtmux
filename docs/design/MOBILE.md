...
## 18. 服务器模式（只读 + 可关，永不可远程开）

Mac 处于服务器模式（合盖不睡）时，手机必须能**持续感知**到 —— 人不在机器旁，否则无从得知它正被
保持唤醒。两处呈现，职责不同：

- **雷达头部的常驻 chip**（摘要行右侧，仅为真时显）：文字「服务器模式」，健康时中性描边，
  护栏触发时转权威红 `#EF4444`。点它进入「分享与设备」。**它不是雷达行** —— 服务器模式是
  机器状态不是 agent 状态，进列表会稀释「谁在等你」。
- **「分享与设备」页顶部的分组**：开启时长 · 电源/电量（并说明「到 20% 自动恢复睡眠」）·
  失效或守护缺失的告警 · **关闭服务器模式**（destructive，二次确认说明后果：合盖会休眠、
  远程连接随之断开）。

**永不提供开启入口。** 开启需要在 Mac 上输入管理员密码，服务器对任何客户端（含已配对的
owner 设备）的 `POST {on:true}` 一律 403。理由不是偷懒:错误的远程开启会让一台无人值守的
笔记本在包里把电耗光,而降权(关闭)永远是安全方向、必须随处可发起。给一个必然失败的开关是撒谎。

**状态与边界**：无服务器模式 → 两处都不显示 · guest token → 连读都 403,完全看不见 ·
远程开启 → 不提供,并说明原因(需在 Mac 上授权)。
