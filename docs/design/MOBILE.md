...
1. **常驻头部只有两行** —— `‹ gtmux HQ ● ` + **一句判断**（有人等你时转琥珀）。就这些。
   ...
   - **舰队 / 用量 / 机器是三个问题，给三行**，各带小字键名、数值用 tabular numerals。
     早先它们挤在一句会折行的灰句子里（`0 need you · 1 working · 16 idle · 5h 21% · wk 53% ·
     Fable 78% disk 16GB free`），**没有任何东西告诉你舰队在哪结束、订阅从哪开始**。
     某一行没数据就整行不出现，不打 `— `；红档时机器行升到常驻，**就不再在下面重复一遍**
     （同一个数字出现在两处，读起来是两件事）。
   - **机器行是核心给的告警时要看得出是告警**（琥珀，不跟其它数字同一个灰）。amber 档按上面
     的规矩留在展开里，但「disk 16GB free」印成跟「5h 24%」一样的灰，读者会把告警当读数。
