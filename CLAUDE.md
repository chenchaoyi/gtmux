...
- **动 HQ 的知识库 / 章程 / `LOCAL.md`**（或想弄清一条经验该落在哪一层）→ 先读 `docs/design/knowledge-layers.md`（三层归属、`promote → land` 的出口、以及「该进代码的那一类」为什么目前只靠自由文本区分）。
