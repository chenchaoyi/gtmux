...
- **`gtmux events [--follow] [--json] [--since <dur>]`**: read (or follow) the
  stream — the terminal-native subscription. `--follow` is ROTATION-AWARE (`tail -F`
  semantics: re-opens when the file is rotated out from under it, so following never
  silently stops); `--since` reads across the rotated generations to cover the
  window. `GET /api/events` already exists for the apps (SSE); this is the CLI side.
