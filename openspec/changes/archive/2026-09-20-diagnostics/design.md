...
### 3.1 Two roots, each meaning one thing

The two directories stay separate. They have different fates: `~/.config/gtmux/` is what
a person set or would carry to a new machine, and is often synced with dotfiles;
`~/.local/share/gtmux/` is what gtmux generates and can regrow. Merging them would sweep
logs, snapshots and uploads into every dotfiles sync and backup along with the
credentials. What was wrong was that each held things of the other kind, the data root
was 60 flat entries of mixed kinds, and only one of the two had a single place in code
that names it.

```
~/.config/gtmux/                   0700   set by a person, or carried to a new machine
  config.json usage.json           0600   settings
  serve-token devices.json …       0600   credentials and identity
  hq/                              0700   HQ's home (stays here, see 3.3)
  knowledge/machine.md             0600   the block every agent reads
  icons/                           0700   hand-placed icon overrides

~/.local/share/gtmux/              0700   generated by gtmux
  ...
  cache/                           0700   regenerable: icon-cache, agent-icons
  tunnel-url                       0600   moved from the config root: it is runtime state
  events.jsonl                     0600   the journal
  active/ waiting/ notify/ …       0700   live state, where it is today
  uploads/ octrans/ mine/ hq-snapshots/  user data and history
```

Moves: `tunnel-url` from the config root to the data root (readers fall back to the old
path for one release); `icon-cache/` and `agent-icons/` into `cache/` (the old ones are
deleted and regrow). Removed: the retired `hq-feed/` and the empty `briefs/`. The
credential backups gtmux left in the config root during earlier migrations
(`*.bak-*`) are offered for removal by `doctor --fix`, not deleted silently.

Live state stays exactly where it is. `active/`, `waiting/`, `last-finished`,
`notify-icon.png` and `notify/` are paths other tools read under the documented contract.

In code, `state.Dir()` already names the data root; `state.ConfigDir()` joins it for the
config root, and every path under either root is built through `state`. The menu bar app
gets the same single place (`Paths.swift`).

### 3.2 Private by default

Everything under both roots is readable by its owner only: files 0600, directories 0700.
Three mechanisms, because one would leave gaps:

- **A private umask at every gtmux entry point.** The Go binary sets `umask 077` before
  anything else, and the menu bar app does the same at launch. Every file gtmux creates is
  then private without each of the 155 call sites that pass a literal mode having to
  change, and a writer added later is private by default.
- **Files gtmux edits that belong to something else** (an agent's settings, the user's
  instruction files, `~/.tmux.conf`, a repository's `AGENTS.md`, a Warp launch config)
  keep their mode: rewriting one in place already preserves it, and creating one goes
  through a helper that sets the conventional mode explicitly.
- **Narrowing on a schedule.** Other processes write under these roots with their own
  umask (the HQ agent writes its board into its home), so serve narrows both trees on
  start and on every disk-hygiene sweep. A file it cannot change (one root created) is
  skipped. `gtmux doctor` checks the result and `--fix` narrows on demand.

### 3.3 HQ's home stays put

`~/.config/gtmux/hq/` is by rights data, not config. It stays: Claude Code files a
project's conversations under its working directory, so moving it would cut HQ off from
its own history; the HQ pane is found by it; and the knowledge page and every agent's
instruction block point at it.
...
- Tests: redaction (4.2), the in-day guard and retention (5), one per serve and tunnel
  event, the command table's `writes` flag against the action catalog (6), the entry
  point setting the private umask, and narrowing a fixture tree (3.2).
- `check-design.sh` fails a path under either gtmux root built outside `internal/state`
  (`".config", "gtmux"` or `".local", "share", "gtmux"` joined by hand) or, in the menu
  bar app, outside `Paths.swift`.
- `check-design.sh` fails a surface that reads `tunnel.log` or any `*.log` for state.
- `gtmux doctor`'s file-modes row checks what is actually on disk (8), which is what
  catches a writer that is not a gtmux process.
