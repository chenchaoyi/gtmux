...
- [ ] 1.1 Add a `hqPlaybookVersion` constant to `internal/app/hq.go` and prepend a
...
- [ ] 1.2 `parsePlaybookVersion(body string) int` — read the version from an AGENTS.md's
  marker (no marker → 0). Pure, unit-tested (marker present / absent / malformed).

## 2. LOCAL.md personalization file

- [ ] 2.1 Add `hqLocalPath()` + a `LOCAL.md` template ("gtmux never overwrites this;
  your instructions here override/extend the managed playbook"). End the generated
  AGENTS.md body with an `@LOCAL.md` import.
- [ ] 2.2 Seed `LOCAL.md` once if absent (seed-if-absent, never overwrite); reseed only
...
- [ ] 3.1 Rework `seedHQHome()`: fresh home → write versioned AGENTS.md + CLAUDE.md import
...
- [ ] 3.2 Legacy migration: an unversioned AGENTS.md (v0) with shipped ≥ 1 → same
  backup+regenerate, with a migration flag so the notice points to LOCAL.md.
- [ ] 3.3 `gtmux hq` prints the one-line upgrade/migration notice (bilingual) when
  seedHQHome reports an upgrade; silent when idempotent.
- [ ] 3.4 Unit-test: fresh seed writes marker+LOCAL.md; newer version upgrades (backup
...
- [ ] 4.1 Update `hqPolicyWarning`/`isClaudePointer` for the managed model (the legacy
  full-CLAUDE.md + dangling-import warnings still apply). Adjust `hq_test.go`.

## 5. Docs, memory, gate

- [ ] 5.1 CLAUDE.md: note the seed lifecycle is now version-tracked — a playbook change
  MUST bump `hqPlaybookVersion` to ship to existing homes; personal edits go in LOCAL.md.
- [ ] 5.2 Update the memory notes that flagged "seed-once won't auto-update"
