...
- [ ] tunnel-worker: server records in the store (id, url, region, label en/zh, accepting,
      restricted), with the single configured address as the implicit server when no list
      exists
- [ ] tunnel-worker: `GET /direct/servers` answers the list a caller may use
- [ ] tunnel-worker: `POST /direct/redeem` takes a server id or region preference and names
      the server it assigned
- [ ] tunnel-worker: the account file endpoint authenticates per server and answers with
      that server's accounts only
- [ ] tunnel-worker: an operator script to add, edit and disable a server without a deploy
- [ ] tunnel-worker tests: assignment by region, a restricted server withheld, per-server
      account files, a legacy deployment with no list behaving as today
- [ ] client: record which server this device is on; `gtmux tunnel --servers` lists them
      with a measured round trip
- [ ] each server answers a liveness path with no Mac behind it (both front-end shapes)
- [ ] docs: `docs/cli.md` + `.zh.md`, the command table, CLAUDE.md's command list
...
- [ ] domain, server id and sync token as parameters
- [ ] a front-end mode for a box whose 443 already belongs to an existing nginx: a site
      template, no second web server installed
- [ ] a download prefix (a mirror) and a locally prepared file as fallbacks for a box that
      cannot reach GitHub, with the pinned SHA-256 verified before install in every case
- [ ] test: a source that serves the wrong bytes is refused, not installed
- [ ] `deploy/self-tunnel/README.md`: adding a server to the pool
