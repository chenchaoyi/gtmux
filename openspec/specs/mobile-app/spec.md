...
The system SHALL further present a card ONLY for a CLEAN, tap-to-reply menu a bare
number-send can drive: a RICH picker (e.g. Claude Code's `AskUserQuestion`, which renders
each option beside a preview panel on the same line so the parsed label swallows the
preview via an interior box rule, and which navigates by arrows/enter rather than a bare
number) is NOT replyable and SHALL get no card. Where no replyable menu can be identified
the system SHALL present no card, leaving the user to reply in their own words / in the
terminal.
