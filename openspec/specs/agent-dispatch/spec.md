...
and branch, for later reclamation), and expose `gtmux tasks [--json]`. A ledger
entry's lifecycle status (delivered → working → waiting → done) SHALL be derived
from the dispatched pane's live radar state. `gtmux tasks` SHALL lead with entries
needing attention (a tracked pane that is waiting or done-after-work), the same
needs-you-first ordering the digest uses.

#### Scenario: A dispatch is tracked

- **WHEN** `gtmux spawn <goal>` succeeds
- **THEN** a ledger entry exists for it and `gtmux tasks` lists it with its live status
